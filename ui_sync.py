...
.ltdl-gemini-key{border-color:#ffe08a!important}.ltdl-gemini-ai{border-color:#8fe1b0!important}
...
.ltdl-key-title{font-size:19px;font-weight:900;margin-bottom:7px}.ltdl-key-note{font-size:13px;color:#607387;line-height:1.55;margin-bottom:12px}
...
.ltdl-key-actions{display:flex;justify-content:flex-end;gap:8px;flex-wrap:wrap;margin-top:12px}.ltdl-key-status{font-size:11px;margin-left:3px}
/* Four compact counters: TN / ĐS / TLN / TL */
...
.dangrow>span:first-child{min-width:0;line-height:1.3;font-size:12px}.dangrow .tag{margin:0 2px!important;padding:3px 7px!important;font-size:10px!important;white-space:nowrap!important;line-height:1.1!important}
@media(max-width:900px){.topin{padding:8px 10px;align-items:flex-start;flex-wrap:wrap;gap:7px}.topin>div:first-child{min-width:0;flex:1 1 100%}.brand{font-size:18px;white-space:normal}.sub{font-size:10px}.nav{width:100%;margin-left:0;justify-content:flex-start;flex-wrap:wrap}.nav a,.nav button{padding:6px 9px;font-size:12px}}
@media(max-width:480px){.topin{padding:7px 8px}.brand{font-size:16px}.sub{font-size:10px}.nav{gap:4px}.nav a,.nav button{padding:6px 7px;font-size:11px}.wrap{padding:8px!important}.qtext{font-size:17px!important}.ltdl-key-box{padding:14px}.ltdl-key-actions{justify-content:stretch}.ltdl-key-actions button{flex:1}}
</style>
'''
...
(function(){
  ...
  function status(){const s=document.getElementById('ltdlKeyStatus');if(s)s.textContent=val()?'✓':'—';const b=document.getElementById('ltdlGeminiKey');if(b)b.title=val()?'Đã nạp Gemini API key':'Nạp Gemini API key';}
  function openKey(){const m=document.getElementById('ltdlKeyModal'),i=document.getElementById('ltdlKeyInput');if(!m||!i)return;i.value=val();m.style.display='flex';setTimeout(()=>i.focus(),30);}
  function closeKey(){const m=document.getElementById('ltdlKeyModal');if(m)m.style.display='none';}
  function saveKey(){const i=document.getElementById('ltdlKeyInput'),msg=document.getElementById('ltdlKeyMsg'),v=(i?.value||'').trim();if(!v){if(msg)msg.textContent='❌ Hãy dán Gemini API key.';return;}if(v.length<20){if(msg)msg.textContent='❌ Gemini API key có vẻ không hợp lệ.';return;}localStorage.setItem(KEY,v);status();if(msg)msg.textContent='✅ Đã lưu key trên trình duyệt này.';setTimeout(closeKey,450);}
  ...
  function studentAnswer(){
    ...
    if(q.kind==='DS' || q.kind==='tf'){
      const arr=[];
      (q.statements||[]).forEach(function(s,i){const z=document.querySelector('input[name=t'+i+']:checked') || document.querySelector('input[name=q'+(q.idx||0)+'_'+i+']:checked');arr.push(z?((z.value==='1'||z.value==='true'||z.value==='Đ')?'Đ':'S'):'?');});
      return arr.join('');
    }
    for(const id of ['ans','shortAns','short0']){const x=document.getElementById(id);if(x)return (x.value||'').trim();}
    return '';
  }
  function standardAnswer(q){
    if(q.answer) return String(q.answer);
    if(q.kind==='TN' || q.kind==='choice'){
      const a=q.options||[];const i=a.findIndex(o=>o.correct===true || o.correct===1 || o.correct==='true');return i>=0?String.fromCharCode(65+i):'';
    }
    if(q.kind==='DS' || q.kind==='tf') return (q.statements||[]).map(s=>(s.correct===true||s.correct===1||s.correct==='true')?'Đ':'S').join('');
    return '';
  }
  async function review(){
    const out=document.getElementById('geminiOut');if(!out){alert('Chưa có vùng hiển thị AI Phản biện.');return;}
    const k=val();if(!k){openKey();return;}
    const q=(typeof Q!=='undefined')?Q:{};
    out.innerHTML='<div class="notice">⏳ Gemini đang phân tích câu hỏi, đáp án chuẩn, bài làm và lời giải...</div>';
    try{
      const r=await fetch('/api/gemini/review_student',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({api_key:k,text:q.text||'',answer:standardAnswer(q),student:studentAnswer(),solution:q.solution||'',kind:q.kind||'',dang:q.dang||'',level:q.level||'',options:q.options||[],statements:q.statements||[]})});
      const d=await r.json();if(!d.ok){out.textContent='❌ '+(d.error||'Gemini không phản hồi.');return;}
      out.textContent=d.text||'';if(window.MathJax&&MathJax.typesetPromise)await MathJax.typesetPromise([out]);const sp=document.getElementById('speakReview');if(sp)sp.style.display='inline-block';
    }catch(e){out.textContent='❌ Không gọi được AI Phản biện: '+e;}
  }
  window.openStudentGeminiKey=openKey;window.closeStudentGeminiKey=closeKey;window.saveStudentGeminiKey=saveKey;window.clearStudentGeminiKey=clearKey;window.askGemini=review;window.askStudentGemini=review;
...
def final_ui_sync(response):
    ...
    try:
        ...
        # One public brand and contact line everywhere.
        ...
        # GitHub is ADMIN-only.
        admin=session.get('role')=='admin';allowed=admin and (request.path.startswith('/admin') or request.path.startswith('/github'))
        if not allowed:
        ...
        # The final layer owns the Gemini UI; old extension modules are no longer imported by wsgi_bootstrap.
        ...
        if is_member_practice:
            if 'id="ltdlGeminiKey"' not in body and "id='ltdlGeminiKey'" not in body:
                ...
                if "class='nav'" in body: body=body.replace("<div class='nav'>","<div class='nav'>"+controls,1)
                elif 'class="nav"' in body: body=body.replace('<div class="nav">','<div class="nav">'+controls,1)

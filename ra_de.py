...
def extract_blocks_by_dang(text: str) -> List[Tuple[str,str,str]]:
    ...
    for m in _BLOCK_RE.finditer(text):
        ...
        name=current or "Chưa phân dạng"; out.append((name,_loai_cau_of_block(m.group(0)),m.group(0)))
    return out
...
def home():
    ...
    for idx,item0 in enumerate(lessons):
        if not isinstance(item0,dict): continue
        item=dict(item0); item['_ra_index']=idx; item['dang_loai']=dang_loai_counts(item.get('path') or '')
...
def _tex_to_word_text(s):
    ...
    for _ in range(6):
        ...
        if old==s: break
    return re.sub(r'\\[a-zA-Z]+\*?(?!\w)','',re.sub(r'\$(.*?)\$',r'\1',s.replace('$$',''),flags=re.S))

def _build_matrix_word_rows(content):
    rows=[]
    if '% MA TRẬN ĐỀ' not in content: return rows
    seg=content.split('% MA TRẬN ĐỀ',1)[1].split(r'\end{tabular}',1)[0]
    for line in seg.splitlines():
        if '&' not in line or 'Bài / Dạng' in line or line.strip().startswith('\\textbf{Tổng}'): continue
        raw=line.replace('\\\\','').replace('\\hline','').strip(); cols=[c.strip() for c in raw.split('&')]
        if len(cols)==6: rows.append([re.sub(r'\\([&%#_{}])',r'\1',c) for c in cols])
    return rows
...
def _build_word_file(content,ten_de):
    ...
    h=doc.add_paragraph(); rr=h.add_run('MA TRẬN ĐỀ'); rr.bold=True; rr.font.size=Pt(13)
    table=doc.add_table(rows=1,cols=6); table.style='Table Grid'
    for cell,label in zip(table.rows[0].cells,['Bài / Dạng','A','B','C','D','Tổng']): cell.text=label; [setattr(run,'bold',True) for run in cell.paragraphs[0].runs]
    for row in _build_matrix_word_rows(content):
    ...
    for code,heading in [('A','PHẦN A. TRẮC NGHIỆM 4 LỰA CHỌN'),('B','PHẦN B. TRẮC NGHIỆM ĐÚNG / SAI'),('C','PHẦN C. TRẢ LỜI NGẮN'),('D','PHẦN D. TỰ LUẬN')]:
        ...
        for i,b in enumerate(grouped[code],1):
            typ,opts,body=_parse_question_block(b); p=doc.add_paragraph(); p.add_run(f'Câu {i}. ').bold=True; p.add_run(body)
            for j,opt in enumerate(opts[:4]): q=doc.add_paragraph(); q.paragraph_format.left_indent=Cm(.7); q.add_run(f'{'ABCD'[j]}. ').bold=True; q.add_run(opt)
            if code=='C': q=doc.add_paragraph('Trả lời: ........................................................................')
...
def download_word(token):
    ...
    safe=re.sub(r'[\\/:*?"<>|]+','_',ten_de).strip() or 'de_thi'

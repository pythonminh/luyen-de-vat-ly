# -*- coding: utf-8 -*-
"""Lightweight GitHub-only question bank UI."""
...
def gh(path, method='GET', payload=None):
    ...
    req=urllib.request.Request(API+path,data=data,method=method,headers={'Authorization':'Bearer '+TOKEN,'Accept':'application/vnd.github+json','X-GitHub-Api-Version':'2022-11-28','User-Agent':'ldvl-bank','Content-Type':'application/json'})
...
def raw(path):
    ...
    with urllib.request.urlopen(urllib.request.Request(u,headers={'User-Agent':'ldvl-bank'}),timeout=10) as q:
        return q.read().decode('utf-8','replace')


def index():
    # Render already has bank_index.json in the repository checkout.
    # Reading local disk is faster and avoids a network request on every catalog load.
    p=os.path.join(APP_DIR,'bank_index.json')
    try:
...
def blocks(s):
    ...
    for n,m in enumerate(pat.finditer(s or ''),1):
        b=m.group(0)
        pm=list(re.finditer(r'\\dangbt\s*\{([^{}]*)\}',s[:m.start()],re.I))
        d=(pm[-1].group(1).strip() if pm else 'Chưa phân dạng')
        ...
        lv=(lm.group(1).strip().upper() if lm else '')
        im=re.search(r'%\s*ID\s*:\s*([^\r\n%]+)',b,re.I)
        qid=(im.group(1).strip() if im else '')
...
# Keep the existing lightweight PAGE from the previous working revision.
PAGE='''
<!doctype html><html lang="vi"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Ngân hàng câu hỏi GitHub</title><style>
...
async function loadCatalog(){try{let r=await fetch('/github/force-catalog',{cache:'no-store'});let txt=await r.text();let j=JSON.parse(txt);if(!j.ok)throw Error(j.error||'Không tải được mục lục');DATA=j.lessons||[];fillFilters();render()}catch(e){document.getElementById('book').innerHTML='<div class="empty">❌ '+esc(e.message||e)+'</div>';document.getElementById('totals').textContent='Lỗi tải'}}
...
def force_catalog():
    try:
        idx=index(); rows=catalog()
        return jsonify({'ok':True,'source':'GitHub','total_files':int(idx.get('total_files') or len(rows)),'total_questions':int(idx.get('total_questions') or sum(x['count'] for x in rows)),'lessons':rows})
...
def force_tex():
    try:
        p=request.args.get('path',''); sha,text=tex(p)
        return jsonify({'ok':True,'path':p,'sha':sha,'text':text,'questions':blocks(text)})
    except Exception as e:
...
def force_save():
    try:
        ...
        if not isinstance(t,str) or not valid(p) or not sha:

...
END_RE=re.compile(r"\\end\s*\{\s*(?:ex|bt)\s*\}",re.I)
...
def kind_of(block):
 if re.search(r'\\choiceTF\b',block or '',re.I): return 'DS'
 if re.search(r'\\choice\b',block or '',re.I): return 'TN'
 if re.search(r'\\shortans\b',block or '',re.I): return 'TLN'
 return 'TL'
def get_header(text):
...
def build():
 ...
 for fp in sorted(BANK_DIR.rglob('*.tex'),key=lambda p:str(p).lower()):
  ...
  lessons.append({'id':rel,'file':rel,'path':rel,'Mon':meta.get('Mon',''),'Lop':meta.get('Lop',''),'Chuong':meta.get('Chuong',''),'BaiHoc':meta.get('BaiHoc',''),'De':meta.get('De',''),'questions':n,'count':n,'dang':d,'dang_kinds':kinds})

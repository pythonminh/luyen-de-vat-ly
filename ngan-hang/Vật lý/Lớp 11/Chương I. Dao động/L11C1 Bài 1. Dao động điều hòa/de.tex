...
% Mức: NB
\immini{
Một chất điểm dao động điều hòa với biên độ $A = 3\,\mathrm{cm}$ và chu kì $T = 1\,\mathrm{s}$. Đồ thị li độ–thời gian như hình bên. Tốc độ trung bình của chất điểm trong một chu kì bằng
}{
\begin{tikzpicture}[>=stealth, line cap=round, line join=round, scale=1.05]
  \draw[->] (-0.15,0) -- (2.35,0) node[below] {$t\,(\mathrm{s})$};
  \draw[->] (0,-1.55) -- (0,1.7) node[left] {$x\,(\mathrm{cm})$};
  \foreach \x/\lab in {0.5/0{,}5,1/1,1.5/1{,}5,2/2}
    \draw (\x,0.06) -- (\x,-0.06) node[below=2pt] {\lab};
  \draw (0.06,1.2) -- (-0.06,1.2) node[left=2pt] {$3$};
  \draw (0.06,-1.2) -- (-0.06,-1.2) node[left=2pt] {$-3$};
  \draw[thick, domain=0:2, samples=200, smooth]
    plot (\x, {1.2*cos(360*\x)});
\end{tikzpicture}
}
\choice
{$24\,\mathrm{cm/s}$}
...
{$3\,\mathrm{cm/s}$}
\loigiai{
Tốc độ trung bình là $v_{\mathrm{tb}} = \dfrac{S}{\Delta t}$, khác với vận tốc trung bình $\vec{v}_{\mathrm{tb}} = \dfrac{\Delta x}{\Delta t}$.

Trong một chu kì, chất điểm đi hết một dao động toàn phần nên quãng đường $S = 4A = 4\cdot 3 = 12\,\mathrm{cm}$ và $\Delta t = T = 1\,\mathrm{s}$. Do đó $v_{\mathrm{tb}} = \dfrac{S}{T} = \dfrac{12}{1} = 12\,\mathrm{cm/s}$. Đây là phương án C.

Các nhiễu thường gặp: $\dfrac{A}{T} = 3\,\mathrm{cm/s}$, $\dfrac{2A}{T} = 6\,\mathrm{cm/s}$, $\dfrac{8A}{T} = 24\,\mathrm{cm/s}$. Vận tốc trung bình trong một chu kì bằng $0$ vì độ dời $\Delta x = 0$.
}
\end{ex}

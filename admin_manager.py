...
def _strict_member_index():
    ...
    body = "<div class='wrap'><div class='panel'><div class='head'>📚 MỤC LỤC <span class='tag'>" + str(idx.get('total_files',0)) + " file</span><span class='tag'>" + str(idx.get('total_questions',0)) + " câu</span></div><div class='body'><div class='notice'>👤 <b>" + html.escape(str(m.get('name') or m.get('username'))) + "</b> · Tài khoản <b>" + html.escape(str(m.get('username'))) + "</b> · 🎓 <b>" + html.escape(display_class) + "</b> · Quyền <b>" + html.escape(typ) + "</b></div><form method='get' class='catsearch'><input name='q' placeholder='Tìm ID câu, bài, chương...' value='" + html.escape(q) + "'><select name='mon'><option value=''>Tất cả môn</option>" + subjopts + "</select><select name='lop'><option value=''>Tất cả lớp</option>" + classopts + "</select><button class='btn'>Tìm</button></form></div></div>" + id_block + (''.join(sections) or ('' if id_block else "<div class='panel' style='margin-top:10px'><div class='body muted'>Không có bài phù hợp với quyền/lớp hiện tại.</div></div>")) + "</div>"

        ...
        for lop in sorted(tree[mon], key=lop_key):
            ...
            for chuong in sorted(tree[mon][lop]):
                ...
                for x in arr:
                    ...
                    dlinks = [
                        f"<a class='drawdang{' on' if on and not current_dang else ''}' href='{html.escape(_go_kind_href(p, '', '', guest), quote=True)}'>Cả bài<span>{n}</span></a>"
                    ]
                    for i, (dname, cnt) in enumerate(pairs, 1):
                        short = dname if len(dname) <= 48 else dname[:47] + "…"
                        on_d = on and current_dang == dname
                        dlinks.append(
                            f"<a class='drawdang{' on' if on_d else ''}' href='{html.escape(_go_kind_href(p, dname, '', guest), quote=True)}' title='{html.escape(dname, quote=True)}'>{i}. {html.escape(short)}<span>{cnt}</span></a>"
                        )
                    bais.append(
                        f"<details class='drawbaiwrap'{' open' if on else ''}>"
                        f"<summary class='drawbai{' on' if on else ''}'>{title}<span>{n}</span></summary>"

...
APP_VERSION = "V13_PRESENTER_NAV_2026_06_02"
SCOPES = [
...
function canShow5050(q){if(!q||!USER.can_5050||SUBMITTED)return false;if(isDungSai(q))return false;if(q.Dang&&q.Dang!='Trắc nghiệm')return false;return['A','B','C','D'].filter(L=>q[L]).length>=2}
function is5050Used(i){return!!(FIFTY_HIDDEN[i]&&FIFTY_HIDDEN[i].length)}
function syncPres5050(q){let show=canShow5050(q),used=is5050Used(CUR);let el=document.getElementById('presBtn5050');if(el){el.classList.toggle('hide',!show);el.disabled=used||(PRES_ON?SUBMITTED:presInputLocked())}
...
async function use5050(fromPres){if(PRES_ON)fromPres=true;if(fromPres)PRES_IGNORE_FS=true;let inPres=!!PRES_ON;try{if(inPres){readAnswerFromUI();renderNav()}else await saveCurrent();let j=await api('/api/fifty',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({sid:SID,index:CUR})});FIFTY_HIDDEN[CUR]=j.hide||[];let btn=document.getElementById('btn5050');if(btn)btn.disabled=true;if(inPres){applyPres5050Hide(FIFTY_HIDDEN[CUR]);if(j.message)showPresToast(j.message)}else{for(let L of FIFTY_HIDDEN[CUR]){let el=document.getElementById('opt_'+L);if(el)el.classList.add('hidden5050')}renderQuestion();if(j.message)alert(j.message)}}catch(e){if(inPres){showPresToast(e.message,true)}else alert(e.message)}finally{setTimeout(()=>{PRES_IGNORE_FS=false},1200)}}

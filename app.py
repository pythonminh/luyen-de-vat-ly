...
function showLearningLoading(kind){
    ...
    hb.innerHTML=`<b>${learningTitle(kind)}</b><div class="hintLoadingPanel"><div class="hintSpinBig"></div><div><b>Đang tải học liệu từ Google Sheet…</b><div class="muted" style="margin-top:6px;font-size:13px">Lấy theo Môn · Lớp · Chương · Bài học${kind==='method'?' · Dạng bài tập':''} của câu hiện tại.</div></div></div>`;
}
function learningFieldHtml(label,val){
    val=String(val||'').trim();
    if(!val)return '';
    return `<div style="margin-top:8px"><b>${esc(label)}:</b><div style="margin-top:3px;line-height:1.55">${formatHintDisplay(val)}</div></div>`;
}
...
async function openLearningPanel(kind){
    ...
    try{
        ...
        let hb=document.getElementById('hintBox');
        if(hb){hb.classList.remove('hide');hb.classList.remove('hintBoxLoading');hb.innerHTML=`<b>${learningTitle(kind)}</b><div class="muted" style="margin-top:8px;color:#991b1b">Không tải được học liệu: ${esc(e.message||e)}</div><div style="margin-top:8px"><button type="button" class="btn2" onclick="openLearningPanel('${kind}')">Thử lại</button></div>`;}
    }
}

...
APP_VERSION = "V279_TRANSLATE_DAPAN_AUTO_2026_06_12"
...
.translateEnReadBar{display:flex;gap:6px;flex-wrap:wrap;align-items:center;margin:6px 0 8px}
.translateEnReadBar .btn2{font-size:12px!important;padding:5px 10px!important;border-radius:999px!important;font-weight:900!important}
.translateEnReadBar .btnReadOn{outline:2px solid #2563eb!important;box-shadow:0 0 0 3px #bfdbfe!important}
...
function backHome(){stopQuizTimer();FS_ANS_FORCE=null;FS_EXP_FORCE=null;MOBILE_NAV_OPEN=false;MOBILE_QUIZ_TOOLS_OPEN=false;VIP_Q_SHOW_ANS={};VIP_Q_SHOW_EXP={};LEARNING_OPEN_KIND='';ASSISTANT_BY_Q={};LEARNING_CACHE={};LEARNING_LOADING=false;ASSISTANT_LOADING=false;TRANSLATE_BY_Q={};TRANSLATE_KIND='CauHoi';TRANSLATE_LOADING=false;TRANSLATE_SIDE_LOADING={};TRANSLATE_AUTO_QUEUE=[];TRANSLATE_SPEECH_KEY='';stopTranslateEnSpeech();unlockQuizPageScroll();updateFilterBadge('','',null);document.getElementById('quiz').classList.add('hide');document.getElementById('home').classList.remove('hide');syncMobileQuizChrome();updateAdminChrome()}
...
function translateEnSpeechPlain(text){let t=String(text||'');t=t.replace(/\$\$[\s\S]*?\$\$/g,' formula ').replace(/\$[^$]*\$/g,' ');t=t.replace(/<[^>]+>/g,' ').replace(/&nbsp;/gi,' ');t=t.replace(/[\\{}_^]/g,' ');return t.replace(/\s+/g,' ').trim()}
function stopTranslateEnSpeech(){if(typeof speechSynthesis!=='undefined'){try{speechSynthesis.cancel()}catch(e){}}TRANSLATE_SPEECH_ACTIVE=false;syncTranslateSpeechBtn()}
function syncTranslateSpeechBtn(){let btn=document.getElementById('btnTranslateRead');if(btn){btn.classList.toggle('btnReadOn',!!TRANSLATE_SPEECH_ACTIVE);btn.textContent=TRANSLATE_SPEECH_ACTIVE?'🔊 Đang đọc…':'🔊 Đọc EN'}}
function speakTranslateEn(text,force){if(typeof speechSynthesis==='undefined'||typeof SpeechSynthesisUtterance==='undefined'){alert('Trình duyệt không hỗ trợ đọc tiếng Anh (Text-to-Speech).');return}let plain=translateEnSpeechPlain(text);if(!plain){alert('Không có văn bản tiếng Anh để đọc.');return}if(TRANSLATE_SPEECH_ACTIVE&&!force)return;stopTranslateEnSpeech();let run=()=>{let u=new SpeechSynthesisUtterance(plain);u.lang='en-US';u.rate=TRANSLATE_SPEECH_RATE||0.92;let voices=speechSynthesis.getVoices?speechSynthesis.getVoices():[];let en=voices.find(v=>/^en(-|_)/i.test(v.lang||''));if(en)u.voice=en;u.onend=()=>{TRANSLATE_SPEECH_ACTIVE=false;syncTranslateSpeechBtn()};u.onerror=()=>{TRANSLATE_SPEECH_ACTIVE=false;syncTranslateSpeechBtn()};TRANSLATE_SPEECH_ACTIVE=true;speechSynthesis.speak(u);syncTranslateSpeechBtn()};let voices0=speechSynthesis.getVoices?speechSynthesis.getVoices():[];if(!voices0.length&&speechSynthesis.onvoiceschanged!==undefined){speechSynthesis.onvoiceschanged=()=>{speechSynthesis.onvoiceschanged=null;run()};return}run()}
...
function speakTranslateEnFromPanel(force){let st=translateEnStore(CUR);let loai=normTranslateKind(TRANSLATE_KIND);let d=st[loai]||{};speakTranslateEn(d.english||d.translation||'',!!force)}
function speakTranslateEnFromKind(kind){let st=translateEnStore(CUR);let d=st[normTranslateKind(kind)]||{};speakTranslateEn(d.english||d.translation||'',true)}
function buildTranslateEnAnswerInline(qIdx,st){if(!canTranslateDapAn(qIdx))return '';let da=st.DapAn||{};if(TRANSLATE_SIDE_LOADING.DapAn)return '<div class="translateEnAnswerBlock"><b>✅ Đáp án (EN)</b><div class="muted" style="margin-top:6px">Đang dịch đáp án…</div></div>';if(!da.translation)return '';let parts=parseTranslateEnParts(da.translation,da.vocabulary,da.notes);return '<div class="translateEnAnswerBlock"><b>✅ Đáp án (EN)</b><div class="hintMath">'+formatHintDisplay(parts.english)+'</div><div style="margin-top:6px"><button type="button" class="btn2" onclick="setTranslateKind(\'DapAn\')">Xem tab Đáp án</button> <button type="button" class="btn2" onclick="speakTranslateEnFromKind(\'DapAn\')">🔊 Đọc đáp án</button></div></div>'}
function maybeAutoReadTranslateEn(qIdx,loai,text){let key=String(qIdx)+'|'+loai;if(TRANSLATE_SPEECH_KEY===key)return;TRANSLATE_SPEECH_KEY=key;setTimeout(()=>{if(LEARNING_OPEN_KIND==='translate'&&CUR===qIdx&&(TRANSLATE_KIND||'CauHoi')===loai)speakTranslateEn(text,false)},350)}
function toggleTranslateSpeechRate(){TRANSLATE_SPEECH_RATE=(TRANSLATE_SPEECH_RATE>=1?0.85:TRANSLATE_SPEECH_RATE+0.05);let lbl=document.getElementById('btnTranslateRate');if(lbl)lbl.textContent='⚡ x'+TRANSLATE_SPEECH_RATE.toFixed(2).replace(/\.00$/,'')}
...
def pwa_service_worker():
    js = """
const CACHE_NAME = 'luyen-de-ai-v279';

...
APP_VERSION = "GOOGLE_SHEET_A_TO_J_FILTER_LAZY_IMAGE_REAL_PROXY_2026_05_31_V15"
...
def normalize_image_url(value: Any) -> Tuple[str, str]:
    ...
    if drive_id:
        ...
        return f"/img/drive/{drive_id}", ""

    if re.match(r'https?://', s):
        return "/img/proxy?u=" + urllib.parse.quote(s, safe=""), ""

    # Trường hợp là đường dẫn tĩnh đã upload chung repo: static/Images/abc.png hoặc Images/abc.png
    local = s.replace("\\", "/").lstrip("/")
    if local.lower().startswith("static/"):
        return "/" + local, ""
    if local.lower().startswith("images/") or local.lower().startswith("img/"):
        return "/static/" + local, "Nếu chưa thấy hình, cần upload thư mục ảnh này lên GitHub trong thư mục static/."

    return "", f"Cột Hình ảnh đang là '{s}'. Cần dán link Drive/file ID hoặc upload ảnh vào static/Images/."
...
EXACT_COLUMN_LABELS = {
    "A": "ID", "B": "BoDe", "C": "De", "D": "Lop", "E": "Mon",
    "F": "Chuong", "G": "BaiHoc", "H": "DangBaiTap", "I": "MucDo",
    "J": "Dang", "K": "NoiDung/CauHoi", "L": "A", "M": "B",
    "N": "C", "O": "D", "P": "DapAn", "Q": "SaiSo",
    "R": "LoiGiai", "S": "HinhAnh", "T": "QuyenTruyCap", "U": "Diem"
}


def header_map(headers: List[str]) -> Dict[str, int]:
    """Map header linh hoạt. Giữ lại để hỗ trợ file cũ, nhưng Cau_Hoi ưu tiên đọc theo cột cố định."""
...
def exact_header_map_for_cau_hoi(headers: List[str]) -> Dict[str, int]:
    """
    Google Sheet của thầy đã chốt cấu trúc cột nên đọc thẳng theo vị trí.
    Việc này tránh sai do tiêu đề có dấu/không dấu hoặc do cột DangBaiTap trống.
    """
    hmap = dict(EXACT_CAU_HOI_COLUMN_MAP)
    # Nếu sau này thầy có thêm cột MaDe thật trong hàng tiêu đề thì vẫn nhận.
    flexible = header_map(headers)
    if "MaDe" in flexible:
        hmap["MaDe"] = flexible["MaDe"]
    return hmap
...
def image_placeholder(message: str):
    ...
    svg = (
        ...
        '<text x="24" y="55" font-family="Arial" font-size="24" fill="#9a3412">Không tải được hình ảnh</text>'
        f'<text x="24" y="100" font-family="Arial" font-size="18" fill="#7c2d12">{msg}</text>'
        '<text x="24" y="138" font-family="Arial" font-size="16" fill="#7c2d12">Kiểm tra cột S:HinhAnh hoặc quyền chia sẻ Drive.</text>'
        '</svg>'
    )
...
def _content_type(resp):
    return resp.headers.get("Content-Type", "") or resp.headers.get("content-type", "") or "application/octet-stream"


def _is_image_response(resp):
    return resp.status_code == 200 and _content_type(resp).lower().startswith("image/") and bool(resp.content)


@app.route("/img/drive/<file_id>")
def img_drive(file_id):
    file_id = clean(file_id)
    if not re.fullmatch(r"[A-Za-z0-9_-]{10,}", file_id):
        return image_placeholder("File ID Drive không hợp lệ")

    try:
        authed = AuthorizedSession(get_google_credentials())
        r = authed.get(f"https://www.googleapis.com/drive/v3/files/{file_id}?alt=media", timeout=20)
        if _is_image_response(r):
            return Response(r.content, mimetype=_content_type(r), headers={"Cache-Control":"public, max-age=3600"})
    except Exception:
        pass
    ...
    for url in [
        ...
        try:
            ...
            continue
    return image_placeholder("Không lấy được ảnh Drive. Share file/folder ảnh cho service account hoặc bật quyền xem bằng link.")
...
def img_proxy():
    ...
    if not re.match(r"^https?://", u):
        return image_placeholder("URL ảnh không hợp lệ")
    try:
        r = requests.get(u, timeout=20, allow_redirects=True, headers={"User-Agent":"Mozilla/5.0"})
        if _is_image_response(r):
            return Response(r.content, mimetype=_content_type(r), headers={"Cache-Control":"public, max-age=3600"})
        return image_placeholder("URL không trả về dữ liệu ảnh")
    except Exception as e:
        return image_placeholder("Lỗi tải ảnh: " + str(e))

@app.route("/version")
def version():
    return jsonify({"version": APP_VERSION, "login_required": True, "admin_can_view_without_submit": True, "admin_can_edit_question": True, "trial_register_3_days": True, "trial_only_free_exam": True, "trial_no_submit_no_score": True, "safe_loading_notice": True, "filter_fix_exact_columns": True, "exact_columns_A_to_J_for_filter": True, "lazy_load_questions_A_to_T_when_start": True, "default_blank_access_is_free": True, "image_drive_link_fix": True, "image_proxy_render": True, "image_column": "S:HinhAnh"})
...
def api_admin_debug():
    ...
    if resp is not None: return resp, status
    with DATA_LOCK:
        return jsonify({"loaded": DATA_CACHE["loaded"], "loading": DATA_CACHE["loading"], "error": DATA_CACHE["error"], "loaded_at": DATA_CACHE["loaded_at"], "count_questions": len(DATA_CACHE["questions"]), "count_catalog": len(DATA_CACHE["catalog"]), "headers": DATA_CACHE["headers"], "header_map": DATA_CACHE["header_map"], "column_mode": DATA_CACHE.get("column_mode"), "exact_columns": DATA_CACHE.get("exact_columns"), "sample_rows": DATA_CACHE["sample_rows"]})
...
def api_admin_update_question():
    ...
    sh = get_sheet(); ws = sh.worksheet("Cau_Hoi"); values = ws.get_all_values(); headers = [clean(x) for x in values[0]]; hmap = exact_header_map_for_cau_hoi(headers); changed = False

...
body.cinema .tfgrid .tf.correct{background:#e8f8ee!important;border:0!important}
body.cinema .tfgrid .tf.wrong{background:#fff0f1!important;border:0!important}
body.cinema .tfgrid .tf.correct .tf-text,body.cinema .tfgrid .tf.ok .tf-text{color:#116a32!important}
body.cinema .tfgrid .tf.wrong .tf-text,body.cinema .tfgrid .tf.noans .tf-text{color:#a41f28!important}
...
def practice():
    ...
    if pos>=len(ids):
        ...
        body=(
            f"<div class='wrap'><div class='panel'><div class='head'>🎉 Kết quả <span class='tag'>Đúng {right}/{len(ids)}</span> <span class='tag'>{score:.2f}/10</span>"
    ...
    body=(f"<div class='wrap'><div class='panel'><div class='head quiztop'><span>📝 Câu {pos+1}/{len(ids)} · <span class='qid'>ID {html.escape(str(q.get('id') or '—'))}</span> · {html.escape(q['dang'])} · {q['kind']}</span>"
          ...
          f"<span>Đúng {right} · Chuỗi {streak} · {html.escape(mode_tag)}</span></div><div class='body'><div class='palette'>{palette}</div><div id='praise'></div>"
...
else if(q.kind==='DS'){h+='<div class="tfgrid"><div class="tf-colhead"><span></span><span class="tf-h yes">Đúng</span><span class="tf-h no">Sai</span></div>';q.statements.forEach((s,i)=>{const lab='ABCD'.charAt(i)||(i+1);h+='<div class="tf" id="t'+i+'"><div class="tf-text">'+lab+') '+s.text+'</div><label class="tf-box yes"><input type="radio" name="t'+i+'" value="1"></label><label class="tf-box no"><input type="radio" name="t'+i+'" value="0"></label></div>'});h+='</div>'}
...
h+='<div class="quizacts"><button class="btn primary" id="chkbtn" onclick="check()" disabled>✅ Xác nhận</button><button id="solbtn" class="btn" style="display:'+(IS_ADMIN?'inline-block':'none')+'" onclick="openSolution()">📖 Xem lời giải</button><button id="next" class="btn" style="display:none" onclick="location.href=\'/member/practice\'">→ Câu tiếp</button></div><div id="hint" class="hintline">'+(IS_ADMIN?'🔐 ADMIN: có thể xem lời giải không cần làm bài.':'Chọn đáp án rồi bấm <b>Xác nhận</b> — lời giải chỉ mở sau khi xác nhận.')+'</div><div id="r">'+(IS_ADMIN?'<div id="solbox" class="solution" style="display:none"><b>📖 Lời giải</b><div>'+(q.solution||'Chưa có lời giải trong file TEX.')+'</div></div>':'')+'</div>';document.getElementById('q').innerHTML=h;ldvlBindQZoom();ldvlApplyQZoom();bind();if(IS_ADMIN)ldvlMountPracticeRewrite()}

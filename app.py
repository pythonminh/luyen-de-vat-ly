...
APP_VERSION = "V277_TRANSLATE_AUTO_GEMINI_2026_06_12"
...
function toggleTranslatePanel(){if(LEARNING_OPEN_KIND==='translate'){closeLearningPanel();return}LEARNING_OPEN_KIND='translate';if(!TRANSLATE_KIND)TRANSLATE_KIND='CauHoi';let hb=document.getElementById('hintBox');if(hb)hb.scrollIntoView({behavior:'smooth',block:'nearest'});requestTranslateEn(TRANSLATE_KIND,false)}
function setTranslateKind(loai){loai=(loai==='LoiGiai')?'LoiGiai':'CauHoi';TRANSLATE_KIND=loai;if(LEARNING_OPEN_KIND==='translate')requestTranslateEn(loai,false)}
function renderTranslatePanel(){let hb=document.getElementById('hintBox');if(!hb||LEARNING_OPEN_KIND!=='translate')return;let qIdx=CUR;let st=translateEnStore(qIdx);let loai=TRANSLATE_KIND||'CauHoi';let data=st[loai]||{};let body='';if(TRANSLATE_LOADING){let loadNote=USER.is_admin?'Đang dịch bằng GPT/Gemini hệ thống…':'Đang dịch bằng Gemini key của bạn…';body='<div class="hintLoadingPanel"><div class="hintSpinBig"></div><div><b>Đang dịch / tải bản dịch…</b><div class="muted" style="margin-top:6px;font-size:12px">'+esc(loadNote)+'</div></div></div>'}else if(data.translation){let cacheNote=data.cached?'<div class="muted" style="font-size:12px;margin-bottom:6px">📋 Đã lưu Sheet Dich_Anh — gọi lại không tốn AI.</div>':'';let provLbl=translateEnProvLabel(data);let prov=provLbl?'<div class="muted" style="font-size:11px;margin-top:6px">'+esc(provLbl)+(data.key_index?' · key #'+data.key_index:'')+'</div>':'';body=cacheNote+'<div class="translateEnPanel"><div class="hintMath">'+formatHintDisplay(data.translation)+'</div></div>'+prov}else{body='<div class="muted" style="margin-top:8px;line-height:1.5">'+(USER.can_ai_hint?(USER.is_admin?'Đang chờ dịch…':'VIP/SVIP: dùng Gemini key của bạn (🔑 Key AI của tôi).'):'Chỉ xem bản dịch đã lưu — VIP/SVIP cần nạp Gemini key để dịch mới.')+'</div>'}let tabQ=loai==='CauHoi'?' translateTabOn':'';let tabL=loai==='LoiGiai'?' translateTabOn':'';let q=QUESTIONS[qIdx]||{};let canLg=canShowSolutionNow()&&!!String(q.LoiGiai||'').trim();let retryBtn=(data.translation&&!TRANSLATE_LOADING)?'<div style="margin:8px 0"><button type="button" class="btn2 translateEnBtn" onclick="requestTranslateEn(TRANSLATE_KIND,true)">🔄 Dịch lại</button></div>':'';let policyNote=USER.is_admin?'<div class="muted" style="font-size:11px;margin-bottom:4px">ADMIN: GPT hoặc Gemini hệ thống.</div>':'<div class="muted" style="font-size:11px;margin-bottom:4px">VIP/SVIP: chỉ Gemini key của bạn — không GPT.</div>';hb.classList.remove('hide');hb.classList.add('learningOpen');hb.innerHTML='<div class="learningTitleRow"><b>🇬🇧 Dịch tiếng Anh</b><button type="button" class="learningCloseBtn" onclick="closeLearningPanel()">✕ Đóng</button></div>'+policyNote+'<div class="translateEnTabs"><button type="button" class="btn2'+tabQ+'" onclick="setTranslateKind(\'CauHoi\')">📝 Đề / phương án</button>'+(canLg?'<button type="button" class="btn2'+tabL+'" onclick="setTranslateKind(\'LoiGiai\')">📗 Lời giải</button>':'')+'</div>'+retryBtn+body+'<div class="aiAssistWarn" style="margin-top:8px">Chỉ dịch & từ vựng — không giải bài, không chốt đáp án.</div>';syncLearningToggleUI();typesetQuizMath()}
async function requestTranslateEn(loai,force){loai=(loai==='LoiGiai')?'LoiGiai':'CauHoi';if(TRANSLATE_LOADING)return;saveCurrent();let qIdx=CUR;let st=translateEnStore(qIdx);if(!force&&st[loai]&&st[loai].translation){TRANSLATE_KIND=loai;LEARNING_OPEN_KIND='translate';renderTranslatePanel();return}TRANSLATE_KIND=loai;LEARNING_OPEN_KIND='translate';TRANSLATE_LOADING=true;renderTranslatePanel();try{let j=await api('/api/translate/en',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({sid:SID,index:qIdx,type:loai,loai:loai,...quizRestorePayload()})});st[loai]={translation:j.translation||'',cached:!!j.cached,vocabulary:j.vocabulary||'',notes:j.notes||'',provider_used:j.provider_used||'',key_index:j.key_index||0,message:j.message||''};if(CUR===qIdx&&LEARNING_OPEN_KIND==='translate')renderTranslatePanel()}catch(e){alert('Không dịch được: '+(e.message||e));if(CUR===qIdx&&LEARNING_OPEN_KIND==='translate')renderTranslatePanel()}finally{TRANSLATE_LOADING=false;if(CUR===qIdx&&LEARNING_OPEN_KIND==='translate')renderTranslatePanel()}}
...
def pwa_service_worker():
    js = """
const CACHE_NAME = 'luyen-de-ai-v277';

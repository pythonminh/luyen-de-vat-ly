...
APP_VERSION = "V193_SVIP_OPENAI_GPT_2026_06_05"
...
def ai_hint_from_provider(
    ...
    def try_openai() -> Tuple[str, int, str, str]:
        nonlocal last_error
        oa_timeout = 70 if admin_review else (28 if svip_hint else 18)
        for idx, api_key in enumerate(openai_keys, start=1):
...
function hintButtonLabel(){if(isAdminViewer()){if(adminUsesGpt())return '🔍 Soát đề GPT';if(adminAiProvider()==='GEMINI')return '🔍 Soát đề Gemini';return '🔍 AI soát đề'}if(svipUsesGpt())return '💡 GPT + đáp án';return USER.is_vip?'💡 AI':'💡 Gợi ý AI'}
function adminQuizStatusLine(){if(!isAdminViewer())return '';if(adminUsesGpt()){let ok=USER.admin_openai_ready!==false;return ok?`ADMIN · Soát đề GPT (${adminAiModelLabel()}) · xem đáp án/lời giải`:`ADMIN · GPT chưa có key Render · xem đáp án/lời giải`}return 'ADMIN: đang xem đáp án/lời giải'}

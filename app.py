...
APP_VERSION = "V12_EXAM_COUNTDOWN_LIVE_2026_06_01"
SCOPES = [
...
<span id="examCountdown" class="exam-strip-cd">Đang tính...</span>
...
function tickExamCountdown(){let tgt=nextExamTarget(),strip=document.getElementById('examStrip'),cd=document.getElementById('examCountdown');if(!cd)return;if(!tgt){if(strip)strip.classList.add('done');cd.textContent='Chúc các em thi tốt! (đã/qua ngày thi 11–12/6)';return}if(strip)strip.classList.remove('done');let parts=formatExamRemain(tgt.t-Date.now());if(!parts){cd.textContent='Sắp đến giờ thi '+tgt.label+'!';return}cd.textContent=`⏳ Còn ${parts.d} ngày ${parts.h} giờ ${pad2(parts.m)} phút ${pad2(parts.s)} giây → thi ${tgt.label}`}

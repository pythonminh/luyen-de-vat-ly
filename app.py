...
APP_VERSION = "V106_DS_ABCD_CIRCLE_DISPLAY_2026_06_06"
...
def build_ai_admin_review_prompt_2026(q: Dict[str, Any], user_answer: Any) -> str:
    ...
    if dang == "Đúng sai":
        dang_note = (
            ...
            "mục 5 liệt kê rõ A=Đúng/Sai, B=..., C=..., D=...; "
            "mục 8 mỗi ý một dòng bắt đầu bằng **A.** **B.** **C.** **D.** rồi Đúng/Sai và giải thích."
        )
    ...
    return "\n".join(
        [
            ...
            "- Với Đúng/Sai: BẮT BUỘC 4 dòng — **A.** Đúng/Sai — giải thích; **B.** ...; **C.** ...; **D.** ...",
...
function extractDsSolutionChunks(text){let t=String(text||'').replace(/\r/g,'');let tagged=[...t.matchAll(/(?:^|\n|\s|[•\-\*]\s*|\(\s*)([ABCD])\s*[\.\):]\s*(Đúng|Sai)\s*[\-—:–]?\s*/gi)];if(tagged.length>=2){let out=[];for(let i=0;i<tagged.length;i++){let start=tagged[i].index+tagged[i][0].length;let end=i+1<tagged.length?tagged[i+1].index:t.length;out.push({letter:tagged[i][1].toUpperCase(),verdict:dsVerdictLabel(tagged[i][2]),body:t.slice(start,end).trim().replace(/^[\)\s]+|[\(\s]+$/g,'')})}return out.slice(0,4)}t=t.replace(/\(\s*•\s*/g,'\n• ').replace(/\s*•\s*/g,'\n• ');let markers=[];let rx=/(?:^|\n|[•\-\*]\s*)(Đúng|Sai)\.\s*/gi,m;while((m=rx.exec(t))!==null)markers.push({verdict:m[1],start:m.index+m[0].length,head:m.index});if(markers.length<2)return [];return markers.map((mk,i)=>{let end=i+1<markers.length?markers[i+1].head:t.length;return {letter:['A','B','C','D'][i]||String(i+1),verdict:dsVerdictLabel(mk.verdict),body:t.slice(mk.start,end).trim().replace(/^[\)\s.,]+|[\(\s.,]+$/g,'')}}).slice(0,4)}
function formatDsSolutionRows(text){let t=String(text||'').trim();if(!t)return '';let chunks=extractDsSolutionChunks(t);if(chunks.length<2)return formatHintDisplay(t);return `<div class="dsSolutionList">${chunks.map(c=>`<div class="dsSolutionItem"><div class="dsSolutionHead">${dsCircleHtml(c.letter)} <b class="${c.verdict==='Sai'?'dsVerdictSai':'dsVerdictDung'}">${esc(c.verdict)}</b></div><div class="dsSolutionBody">${formatHintDisplay(c.body)}</div></div>`).join('')}</div>`}
function formatDsHintText(text,isSolution){text=String(text||'').trim();if(!text)return '';return isSolution?formatDsSolutionRows(text):formatDsAnswerBadges(text)}
function isCurrentQuestionDs(){let q=QUESTIONS[CUR];return !!(q&&applyResolvedDang(q).Dang==='Đúng sai')}
...
function buildHintAnswerCard(j){if(!j)return '';if(!j.show_answer&&!j.vip_detailed&&!j.exact&&!j.admin_review)return '';let aiDa=String(j.suggested_dapan||j.correct||'').trim();let sheetDa=String(j.sheet_dapan||'').trim();let aiLg=String(j.suggested_loigiai||'').trim();let sheetLg=String(j.sheet_loigiai||'').trim();if(!aiDa&&!sheetDa&&!aiLg&&!sheetLg)return '';let isDs=isCurrentQuestionDs();let fmtAns=isDs?formatDsHintText:formatHintDisplay;let fmtSol=isDs?formatDsHintText:null;let rows='';if(sheetDa||aiDa){rows+=`<div class="hintAnswerRow"><b>📌 Đáp án đúng:</b> <span class="hintMath hintAnswerMain">${isDs?fmtAns(aiDa||sheetDa,false):formatHintDisplay(aiDa||sheetDa)}</span></div>`;if(sheetDa&&aiDa&&sheetDa.replace(/\s/g,'')!==aiDa.replace(/\s/g,''))rows+=`<div class="hintAnswerRow muted" style="font-size:13px"><b>Sheet (P):</b> <span class="hintMath">${isDs?fmtAns(sheetDa,false):formatHintDisplay(sheetDa)}</span> · <b>AI:</b> <span class="hintMath">${isDs?fmtAns(aiDa,false):formatHintDisplay(aiDa)}</span></div>`}let lg=aiLg||sheetLg;if(lg)rows+=`<div class="hintAnswerRow" style="margin-top:8px"><b>📝 Lời giải:</b> <span class="hintMath">${isDs?fmtSol(lg,true):formatHintDisplay(lg)}</span></div>`;if(!rows)return '';return `<div class="hintAnswerCard"><div class="hintAnswerTitle">✅ Đáp án &amp; lời giải — so sánh ôn tập</div>${rows}</div>`}

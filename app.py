...
VIDEO_CMD_RE=re.compile(r'\\(?:video|youtube|clip)\s*(?:\[(?P<title>[^\]]*)\])?\s*\{(?P<url>[^{}]*)\}',re.I)
...
def latex_to_web(s):
    ...
    def stash_video(m):
        g=m.groupdict()
        vid=youtube_id(g.get('url') or m.group(0))
        if not vid: return ''
        vids.append(video_html(vid, g.get('title') or ''))

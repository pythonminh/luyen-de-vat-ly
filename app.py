...
def _infographic_notebook_style_rules() -> List[str]:
    """Quy tắc trình bày vở học sinh + ký hiệu khoa học cho prompt Gemini tạo ảnh."""
    return [
        ...
        "• KHÔNG làm poster/digital flashy — phải giống ảnh chụp/scan một TRANG VỞ đã giải xong",
        "",
...
def version():
    return jsonify({
        ...
        "retry_shuffle": True,
        "routes": ["/login", "/register", "/logout", "/share", "/d/<made>", "/api/meta", "/api/start", "/api/submit", "/api/question/create", "/api/question/update", "/api/question/delete", "/api/question/dedupe", "/api/question/lookup", "/api/infographic-prompt"]
    })

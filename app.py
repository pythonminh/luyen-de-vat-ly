...
else if(q.kind==='DS'){h+='<div class="tfgrid"><div class="tf-colhead"><span></span><span class="tf-h yes">Đúng</span><span class="tf-h no">Sai</span></div>';q.statements.forEach((s,i)=>{const lab='abcd'.charAt(i)||(i+1);h+='<div class="tf" id="t'+i+'"><div class="tf-text"><b>'+lab+')</b> '+s.text+'</div><label class="tf-box yes"><input type="radio" name="t'+i+'" value="1"></label><label class="tf-box no"><input type="radio" name="t'+i+'" value="0"></label></div>'});h+='</div>'}
...
function verdictHtml(q, student, ok){
  ...
  if(q.kind==='TN'){
    ...
    return head+'<div class="keyline">ĐÁP ÁN ĐÚNG: '+key+'</div><div class="keyline">BẠN CHỌN: '+(student||'—')+'</div>';
  }
  if(q.kind==='DS'){
    ...
    const pick=(q.statements||[]).map(function(s,i){return labs.charAt(i)+'-'+(marks[i]||'?')}).join(', ');
    return head+'<div class="keyline">ĐÁP ÁN ĐÚNG: '+key+'</div><div class="keyline">BẠN CHỌN: '+pick+'</div>';
  }
  if(q.kind==='TLN'){

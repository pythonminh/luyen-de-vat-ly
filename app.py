...
APP_VERSION = "V13_PRESENTER_5050_FIX_2026_06_02"
SCOPES = [
...
function presInputLocked(){if(SUBMITTED)return true;if(PRES_ON)return false;let r=getQResult(CUR);return!!r&&r.ok!=null&&!USER.is_admin}
...
function syncPres5050(q){let show=canShow5050(q),used=is5050Used(CUR);let el=document.getElementById('presBtn5050');if(el){el.classList.toggle('hide',!show);el.disabled=used||(PRES_ON?SUBMITTED:presInputLocked());el.textContent=used?'Đã dùng 50-50':'50-50'}}
...
async function saveCurrent(){let q=QUESTIONS[CUR];if(!q||USER.is_admin)return;readAnswerFromUI();renderNav();await checkCurrentQuestion();if(PRES_ON){let r=getQResult(CUR);if(!r||r.ok==null)renderPresenter()}}

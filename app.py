...
def public_origin():
    """Link Chiếu / QR luôn trỏ domain đang dùng cho lớp."""
    want = PUBLIC_HOST
    host = (request.host or "").split(":")[0].lower()
    if want and (host == want or host in LEGACY_HOSTS):
        return "https://" + want
...
def _canonical_public_host():
    ...
    if host not in LEGACY_HOSTS:
        return None
    dest = "https://" + want + request.full_path
    if dest.endswith("?"):
        dest = dest[:-1]
...
.ltbox.example,.ltbox.vd{background:#f3fdf6!important;border:1px solid #86efac!important;border-left:5px solid #16a34a!important}
.ltbox.example .k,.ltbox.vd .k{background:#dcfce7!important;color:#166534!important}
...
html:has(body.cinema){scroll-padding-top:0;color-scheme:only light}
body.cinema{color-scheme:only light}
...
body.cinema .cinema-inktools{display:flex;flex-wrap:wrap;gap:6px;align-items:center;margin:0 0 6px;font-size:12px;font-weight:800;color:#9a6b3a}
body.cinema .cinema-inktools .inkpaper{border:1px solid #f0c7a0;background:#fff;color:#9a3412;border-radius:8px;padding:4px 8px;cursor:pointer;font:inherit;font-weight:800}
body.cinema .cinema-inktools .inkpaper.on{background:#9a3412;color:#fff;border-color:#9a3412}
body.cinema .cinema-inkframe{position:relative;width:100%;height:var(--ink-h,220px);min-height:140px;max-height:90vh;padding-bottom:0;background:#fffdf6;border:2px solid #e8c39a;border-radius:12px;overflow:hidden;color-scheme:only light;forced-color-adjust:none;touch-action:none}
body.cinema .cinema-inkframe[data-paper="lines"]{background-color:#fffdf6;background-image:linear-gradient(to right,transparent 0 40px,#e11d48 40px,#e11d48 42px,transparent 42px),repeating-linear-gradient(#fffdf6 0 32px,#b45309 32px,#b45309 34px)}
body.cinema .cinema-inkframe[data-paper="grid"]{background-color:#fffdf6;background-image:repeating-linear-gradient(to right,transparent 0 26px,#b45309 26px,#b45309 28px),repeating-linear-gradient(to bottom,transparent 0 26px,#b45309 26px,#b45309 28px)}
body.cinema .cinema-inkframe[data-paper="plain"]{background:#fffdf6;background-image:none}
body.cinema .cinema-ink{position:absolute!important;left:0;top:0;right:0;bottom:0;inset:0;width:100%!important;height:100%!important;max-width:100%;max-height:100%;z-index:0;touch-action:none;pointer-events:none;display:block;background-color:#fffdf6!important;color-scheme:only light;forced-color-adjust:none}
body.cinema.is-host.ink-on .cinema-ink{pointer-events:auto;cursor:crosshair}
body.cinema .cinema-inkresize{height:22px;margin-top:2px;border:1px dashed #f0c7a0;border-radius:8px;background:#fff7ed;color:#c2410c;font-size:11px;font-weight:800;display:flex;align-items:center;justify-content:center;cursor:ns-resize;user-select:none;touch-action:none}
...
body.cinema .cinema-qr img{width:56px;height:56px;display:block;background:#fff;padding:3px;border-radius:6px;box-sizing:border-box}
body.cinema .cinema-qr.is-min img{width:36px;height:36px}
...
body.cinema .spkchunk,body.cinema .spkhost>.spkchunk,body.cinema button.spkchunk{display:none!important;width:0!important;height:0!important;overflow:hidden!important;margin:0!important;padding:0!important;border:0!important}
body.cinema .tf-box.ok{border-color:#15803d!important;background:#ecfdf3!important}
body.cinema .tf-box.ok::after{content:"";width:.5em;height:.5em;border-radius:1px;background:#15803d!important}
body.cinema .tf-box.bad{border-color:#b91c1c!important;background:#fef2f2!important}
body.cinema .tf-box.bad::after{content:"";width:.5em;height:.5em;border-radius:1px;background:#b91c1c!important}
body.cinema .opt.correct{background:#ecfdf3!important;border:2px solid #15803d!important}
body.cinema .opt.wrong{background:#fef2f2!important;border:2px solid #b91c1c!important}
...
body.cinema .cinema-votes .vgroups{margin-top:4px}
body.cinema .cinema-votes .vgrp{display:grid;grid-template-columns:6.4em minmax(0,1fr);gap:6px;align-items:start;padding:2px 0;font-size:12px}
body.cinema .cinema-votes .vgrp b{color:#1e3a8a;white-space:nowrap}
body.cinema .cinema-votes .vgrp span{color:#0f172a}
...
body.cinema .cinema-namegate .namegate-card{width:min(400px,100%);padding:18px 16px;border-radius:14px;background:#fff;border:1px solid #c5d6ea;box-shadow:0 12px 40px #0005}
body.cinema .cinema-namegate h2{margin:0 0 6px;font-size:18px;color:#0f172a}
body.cinema .cinema-namegate p{margin:0 0 12px;font-size:13px;color:#475569;line-height:1.4}
...
body.cinema .cinema-violate.is-ok{border-color:#86efac;background:#ecfdf3;color:#166534}
body.cinema .cinema-violate .vt{font-size:14px;font-weight:900;margin:0 0 6px}
body.cinema .cinema-violate .vr{margin:3px 0}
body.cinema .cinema-violate .vr b{color:#7f1d1d}
body.cinema .cinema-violate.is-ok .vr b{color:#166534}
body.cinema .cinema-violate.is-seal{border-color:#7f1d1d;background:#450a0a;color:#fecaca}
body.cinema .cinema-violate.is-seal .vr b,body.cinema .cinema-violate .seal{color:#fecaca}
body.cinema .cinema-violate .seal{margin-top:6px;font-weight:900;letter-spacing:.02em}
body.cinema .votetip.seal{border-color:#b91c1c;background:#fef2f2;color:#991b1b}
body.cinema .cinema-violate-seal{margin:8px 12px;padding:14px 12px;border:3px solid #b91c1c;border-radius:12px;background:#7f1d1dee;color:#fff;text-align:center;font-weight:800;z-index:25;position:relative}
body.cinema .cinema-violate-seal .seal-stamp{display:inline-block;margin:0 0 6px;padding:6px 12px;border:3px solid #fecaca;border-radius:8px;transform:rotate(-6deg);font-size:16px;font-weight:900;letter-spacing:.04em;color:#fecaca;background:#450a0a}
...
body.cinema .cinema-peek-sol{margin-top:6px;font-weight:400;color:#1e293b;font-size:15px;line-height:1.55}
...
.present-host.has-code #pPrevQuick,.present-host.has-code #pNextQuick{display:inline-flex;align-items:center;justify-content:center;margin-left:0}
.present-host .presentspeak{flex:1 1 100%}
@media(max-width:700px){
.present-host{gap:4px;padding:4px 6px}
.present-host #pStart{flex:1 1 auto;min-height:44px}
.present-host #pCode{font-size:20px}
.present-host #pPrevQuick,.present-host #pNextQuick{min-width:44px;min-height:44px;font-size:16px;padding:6px 10px}
.present-host #navFold{flex:0 0 auto}
.present-host .presentspeak,.present-host #presentBar{flex:1 1 100%;max-height:22vh}
}
...
.quizacts .btn{flex:1 1 0;min-height:40px;font-size:14px;display:inline-flex;align-items:center;justify-content:center}
...
def page(title: str, body: str, cinema: bool = False) -> Response:
    ...
    mj = (
        ...
        "window.ldvlNavIsDesktop=function(){return window.matchMedia('(min-width:1025px)').matches};"
        "window.ldvlNavApply=function(on){var top=document.querySelector('.top');if(!top||!tb)return;"
        "if(ldvlNavIsDesktop())on=false;"
        "top.classList.toggle('nav-open',!!on);tb.setAttribute('aria-expanded',on?'true':'false');tb.textContent=on?'✕':'☰';"
        "ldvlSyncHeaderH();};"
        "if(tb&&nv){ldvlNavApply(false);tb.onclick=function(){if(ldvlNavIsDesktop())return;"
        "ldvlNavApply(!document.querySelector('.top').classList.contains('nav-open'));};"
        "window.addEventListener('resize',function(){if(ldvlNavIsDesktop())ldvlNavApply(false);});}"
...
_TEX_SRC = contextvars.ContextVar("tex_src", default="")
IMG_EXTS = (".png", ".jpg", ".jpeg", ".webp", ".gif")
LESSON_IMAGE_DIRS = ("images", "ImagesGPT", "Images")
INC_CMD_RE = re.compile(r"\\includegraphics(?:\s*\[(?P<opt>[^\]]*)\])?\s*", re.I)
WRAPFIG_RE = re.compile(r"\\begin\s*\{\s*wrapfigure\s*\}.*?\\end\s*\{\s*wrapfigure\s*\}", re.I | re.S)
...
def _fetch_bytes_remote(path):
    p, _ = _safe_repo_file(path)
    raw_url = f"https://raw.githubusercontent.com/{REPO}/{urllib.parse.quote(BRANCH, safe='')}/{urllib.parse.quote(p, safe='/')}"
    headers = {"User-Agent": "luyen-de-vat-ly-clean", "Cache-Control": "no-cache", "Pragma": "no-cache"}
    if TOKEN:
...
def _graphic_name_variants(name):
    name = _norm_graphic_name(name)
    ...
    if ext.lower() in IMG_EXTS:
        out.extend([name, base])
    else:
        for e in IMG_EXTS:
            out.extend([name + e, stem + e, base + e, base_stem + e])
        out.extend([name, base])
    if "/" in name:
        out.append(name.split("/", 1)[-1])
        leaf = name.rsplit("/", 1)[-1]
        leaf_stem, leaf_ext = os.path.splitext(leaf)
        if leaf_ext.lower() in IMG_EXTS:
            out.append(leaf)
        else:
            for e in IMG_EXTS:
                out.append(leaf + e)
                out.append(leaf_stem + e)
    seen, uniq = set(), []
    for x in out:
        x = str(x).replace("\\", "/").lstrip("/")
        if x and x not in seen and ".." not in x.split("/"):
            seen.add(x)
            uniq.append(x)
    return uniq


def _candidate_image_rels(name, tex_path=""):
    tex_path = str(tex_path or _TEX_SRC.get() or "").replace("\\", "/")
    folder = lesson_folder(tex_path) if tex_path else ""
    names = _graphic_name_variants(name)
    roots = []
    if folder.startswith("ngan-hang/"):
        roots.append(folder)
        for d in LESSON_IMAGE_DIRS:
            roots.append(folder + "/" + d)
    roots.append("ngan-hang")
    rels, seen = [], set()
    for root in roots:
        for n in names:
            rel = (root.rstrip("/") + "/" + n).replace("//", "/")
            if not rel.startswith("ngan-hang/") or ".." in rel.split("/"):
                continue
            if rel not in seen:
                seen.add(rel)
                rels.append(rel)
...
def resolve_lesson_image(name, tex_path=""):
    rels = _candidate_image_rels(name, tex_path)
    for rel in rels:
        try:
            p, local = _safe_repo_file(rel)
        except Exception:
            continue
        if local.is_file() and local.stat().st_size > 20:
            return p, local
    if not (_on_render() or os.getenv("FORCE_GITHUB_TEX") == "1"):
        return "", None
    for rel in rels[:8]:
        try:
            p, local = _safe_repo_file(rel)
            blob = _fetch_bytes_remote(p)
        except Exception:
            continue
        if blob and len(blob) > 20 and blob[:15] != b"404: Not Found" and blob[:9] != b"Not Found":
            try:
                local.parent.mkdir(parents=True, exist_ok=True)
...
def includegraphics_html(opt, raw_name):
    rel, _local = resolve_lesson_image(raw_name)
    shown = html.escape(_norm_graphic_name(raw_name) or "ảnh")
    if not rel:
        return (
            f"<span class='tex-img-miss'>Thiếu ảnh <b>{shown}</b>. "
            "Đặt file png/jpg vào thư mục <b>images/</b> của bài (cạnh de.tex) rồi đẩy GitHub.</span>"
        )
    url = "/bank-img/" + urllib.parse.quote(rel[len("ngan-hang/"):], safe="/")
    style = ""
    m = re.search(r"width\s*=\s*([\d.]+)\s*cm", str(opt or ""), re.I)
    if m:
        px = int(round(float(m.group(1)) * 38))
        style = f" style='max-width:{max(80, min(px, 420))}px'"
    return (
        f"<img class='tex-img' src='{html.escape(url, quote=True)}' alt='{shown}'{style} "
        "loading='lazy' decoding='async'>"
    )


def _replace_includegraphics(s, bucket):
    out, i = [], 0
    while True:
        m = INC_CMD_RE.search(s, i)
        if not m:
            out.append(s[i:])
            break
        arg, end = get_braced(s, m.end())
        if arg is None:
            out.append(s[i:m.end()])
            i = m.end()
            continue
        bucket.append(includegraphics_html(m.group("opt") or "", arg))
        out.append(s[i:m.start()] + f"@@INC{len(bucket)-1}@@")
        i = end
    return "".join(out)
...
def html_question(s, tex_path=None):
    token = None
    if tex_path:
        token = _TEX_SRC.set(str(tex_path).replace("\\", "/"))
    try:
...
def convert_list_env(s, name, tag, stash):
    ...
    while True:
        ...
        for it in items:
            ...
            if lab:
                lis.append(f'<li><span class="tex-item-lab">{html.escape(lab)}</span> {inner}</li>')
            else:
...
def box_k_html(kind, word):
    n = int(_BOX_SEQ.get(kind) or 0) + 1
    _BOX_SEQ[kind] = n
    return n, f"<span class='lt-num'>{n}</span>{html.escape(word)} {n}"

def convert_pedagogic_envs(s):
    """Gỡ \\begin{vd|dl|lt|...} thành token, không để lộ lệnh thô trên web."""
    boxes = []
    s = s or ''
    for _ in range(80):
        best = None
        meta = None
        for name, cls, label in PEDAGOGIC_ENVS:
            hit = _match_tex_env(s, name)
        ...
        a0, a1, b0, b1 = best
        tok = f'@@PED{len(boxes)}@@'
        boxes.append((meta[0], meta[1], meta[2], s[a1:b0]))
        s = s[:a0] + tok + s[b1:]
    s = re.sub(r'\\begin\s*\{\s*(?:vd|dl|lt|dn|nx)\s*\}', '', s, flags=re.I)
    s = re.sub(r'\\end\s*\{\s*(?:vd|dl|lt|dn|nx)\s*\}', '', s, flags=re.I)
    return s, boxes

def latex_to_web(s):
    """HTML cho web: TikZ → hình, ảnh bài → <img>, tabular → bảng, còn lại $...$ cho MathJax."""
    figs=[]; bolds=[]; italics=[]; tabs=[]; lists=[]; vids=[]; incs=[]; wraps=[]
    def stash_tikz(m):
    ...
    def stash_wrap(m):
        inner = m.group(0)
        found = []
        _replace_includegraphics(inner, found)
    ...
    s=_replace_includegraphics(s, incs)
    ...
    for i,ls in enumerate(lists):
        put(f'@@LST{i}@@', ls)
    for i,(name,cls,label,body) in enumerate(peds):
        inner = latex_to_web(body)
        _, khtml = box_k_html(name, label)
        put(
            f'@@PED{i}@@',
            f"<div class='ltbox {cls}'><div class='k'>{khtml}</div>"
    ...
    s=re.sub(r'\\begin\s*\{\s*(?:tabular|center|tikzpicture|figure|minipage|itemchoice|itemize|enumerate|wrapfigure|vd|dl|lt|dn|nx)\s*\}(?:\{[^{}]*\})?','',s,flags=re.I)
    s=re.sub(r'\\end\s*\{\s*(?:tabular|center|tikzpicture|figure|minipage|itemchoice|itemize|enumerate|wrapfigure|vd|dl|lt|dn|nx)\s*\}','',s,flags=re.I)
    s=re.sub(r'\\(?:immini|resizebox)(?:\s*\[[^\]]*\])?(?:\s*\{[^{}]*\}){0,3}\s*\{?','',s,flags=re.I)
    s=re.sub(r'%\s*\[[^\]]+\]','',s)
    if any(wraps):
        figs_html=''.join(f"<div class='immini-fig'>{w}</div>" for w in wraps if w)
        s=f"<div class='immini'><div class='immini-txt'>{s}</div>{figs_html}</div>"
    return s
...
def practice():
    ...
    if pos>=len(ids):
        ...
        if has_full_bank_access(m):
            from live_present import PRESENT_TTS_JS
            extra_js=PRESENT_TTS_JS
...
function ldvlRemountSpeak(){if(!(window.ldvlSpeak&&window.ldvlSpeak.mountChunks))return;window.ldvlSpeak.mountChunks(document.getElementById('q'));let pane=document.getElementById('aipane');if(pane)window.ldvlSpeak.mountChunks(pane)}
function keyHint(q){
  const labs='ABCDEFGH';
  if(q.kind==='TN') return (q.options||[]).map(function(o,i){return o.correct?labs.charAt(i):''}).filter(Boolean).join('')||'—';
  if(q.kind==='DS') return (q.statements||[]).map(function(s,i){return labs.charAt(i)+(s.correct?'Đ':'S')}).join(' ')||'—';
  if(q.kind==='TLN') return q.answer||'—';
  const t=String(q.solution||'').replace(/<[^>]+>/g,' ').replace(/\s+/g,' ').trim();
...
function toggleHint(){
  ...
  if(!box){
    if(!r) return;
    r.insertAdjacentHTML('afterbegin','<div id="hintbox" class="adminhint" style="display:none"></div>');
    box=document.getElementById('hintbox');
  }
  if(box.style.display==='block'){box.style.display='none';return;}
  box.innerHTML='<b>💡 Gợi ý (chỉ thầy — lớp chưa thấy)</b>'+E(keyHint(Q));
...
@app.route('/bank-img/<path:rel>')
def bank_img(rel):
    """Ảnh trong thư mục images/ (hoặc ImagesGPT/) của từng bài."""
    p = 'ngan-hang/' + str(rel or '').replace('\\', '/').lstrip('/')
    if Path(p).suffix.lower() not in IMG_EXTS:
        abort(404)
    try:
        p, local = _safe_repo_file(p)
    except Exception:
        abort(404)
    ...
    resp = send_file(local, mimetype=mime, conditional=True)

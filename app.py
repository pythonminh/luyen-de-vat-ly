...
APP_VERSION = "V91_LATEX_TABULAR_HTML_2026_06_05"
...
function uniqField(list,field){let seen=new Set(),out=[];for(let x of list||[]){let v=String((x||{})[field]||'').trim();if(!v||seen.has(v))continue;seen.add(v);out.push(v)}return out.sort((a,b)=>normText(a).localeCompare(normText(b),'vi'))}

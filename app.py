...
APP_VERSION = "V287_ADMIN_DANGBAITAP_SELECT_2026_06_12"
...
function adminDangBaiTapSuggestions(){try{let a=(META&&META.dangbaitap_suggestions)||((META&&META.filters&&META.filters.DangBaiTap)||[]);return a.slice(0,120)}catch(e){return []}}
function adminDangBaiTapOptions(q){let seen={},out=[];function add(v){v=String(v||'').trim();if(!v)return;let k=normText(v);if(seen[k])return;seen[k]=1;out.push(v)}for(let x of adminDangBaiTapSuggestions())add(x);let qMon=normText((q&&q.Mon)||''),qBai=normText((q&&q.BaiHoc)||'');for(let c of (CATALOG||[])){if(qMon&&normText(c.Mon)!==qMon)continue;if(qBai&&normText(c.BaiHoc)!==qBai)continue;let fc=(c.FilterCounts||{}).dangbaitap||{};if(fc&&typeof fc==='object'&&!Array.isArray(fc))Object.keys(fc).forEach(add);let raw=String(c.DangBaiTap||'');if(raw)for(let part of raw.split(/[,;|]+/))add(part.trim())}for(let qq of (QUESTIONS||[])){if(qMon&&normText(qq.Mon)!==qMon)continue;if(qBai&&normText(qq.BaiHoc)!==qBai)continue;add(qq.DangBaiTap)}add(q&&q.DangBaiTap);return out.sort((a,b)=>a.localeCompare(b,'vi'))}
...
function adminDbtSyncSelectFromInput(){let inp=document.getElementById('edit_DangBaiTap');let sel=document.getElementById('edit_DangBaiTap_pick');if(!inp||!sel)return;let raw=String(inp.value||'').trim();let opts=[];for(let i=0;i<sel.options.length;i++){let v=sel.options[i].value;if(v&&v!=='__custom__')opts.push(v)}let m=adminDbtFindMatch(raw,opts);if(m){sel.value=m;inp.value=m;inp.classList.add('hide')}else if(raw){if(!Array.from(sel.options).some(o=>o.value===raw)){let o=document.createElement('option');o.value=raw;o.textContent=raw;let customOpt=sel.querySelector('option[value="__custom__"]');if(customOpt)sel.insertBefore(o,customOpt);else sel.appendChild(o)}sel.value='__custom__';inp.classList.remove('hide')}else{sel.value='';inp.classList.remove('hide')}adminDbtCheckDuplicateHint()}
function renderAdminDangBaiTapField(q){let raw=String((q&&q.DangBaiTap)||'').trim();let opts=adminDangBaiTapOptions(q);let match=adminDbtFindMatch(raw,opts);let pickVal=match||(raw?'__custom__':'');let optHtml='<option value="">— Chọn dạng có sẵn —</option>';for(let v of opts){let sel=(pickVal===v)?' selected':'';optHtml+=`<option value="${escAttr(v)}"${sel}>${esc(v)}</option>`}let customSel=(pickVal==='__custom__')?' selected':'';optHtml+=`<option value="__custom__"${customSel}>✏️ Nhập / sửa dạng khác…</option>`;let hideInp=(pickVal&&pickVal!=='__custom__')?' hide':'';return `<div class="adminQuickField adminDbtField"><label><b>${QUESTION_FORM_LABELS.DangBaiTap}</b></label><select id="edit_DangBaiTap_pick" class="adminDbtSelect" onchange="adminDbtPickChange()">${optHtml}</select><input type="text" id="edit_DangBaiTap" class="adminDbtInput${hideInp}" value="${escAttr(raw)}" placeholder="Gõ dạng bài tập mới (tránh trùng tên gần giống)" oninput="adminDbtInputChange()" /><div id="edit_DangBaiTap_hint" class="muted adminDbtHint">${pickVal&&pickVal!=='__custom__'?'Đã chọn dạng có sẵn — tên thống nhất.':'Chọn trong danh sách hoặc gõ tên mới.'}</div><div id="edit_DangBaiTap_dup" class="adminDbtDup hide"></div></div>`}
...
def pwa_service_worker():
    js = """
const CACHE_NAME = 'luyen-de-ai-v287';

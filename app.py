...
APP_VERSION = "V29_FULL_NAV_REAL_QUESTION_LIST_2026_06_02"
...
<script>
let META=null,CATALOG=[],USER={},SID='',QUESTIONS=[],CUR=0,ANSWERS={},SUBMITTED=false,RESULTS={},CHECKED={},LOCKED_Q={},CURRENT_MADE='',START_IS_RETRY=false,QUIZ_ELAPSED=0,QUIZ_TIMER=null,FS_ANS_FORCE=null,FS_EXP_FORCE=null,FULLDE_ON=false,FS_NAV_HIDDEN=false;
...
function ensureNavInfo(){
  ...
  if(title) title.textContent='Danh sách câu hỏi';
...
function ensureNavList(){
  ...
  if(!list){
    ...
    list.id='navList';
    list.style.cssText='margin-top:8px;display:flex;flex-direction:column;gap:4px;overflow:auto;max-height:38vh;padding-right:2px';
    let nav=document.getElementById('navNums');
    if(nav && nav.parentNode) nav.parentNode.insertBefore(list,nav.nextSibling);
  }
  return list;
}

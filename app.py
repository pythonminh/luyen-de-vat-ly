...
APP_VERSION = "V13_LOCK_ANSWERED_2026_06_02"
SCOPES = [
...
def score_dungsai_detail(q: Dict[str, Any], user_answer: Any) -> Dict[str, Any]:
    """Chấm Đúng/Sai + chi tiết từng ý A-D (đúng với lựa chọn học sinh)."""
    ...
    for i, L in enumerate(["A", "B", "C", "D"]):
        ...
        if not c:
            continue
        ok_row = bool(ch) and c == ch
        if ch:
            checks.append(ok_row)
        rows.append({"letter": L, "chosen": ch or "", "correct": c, "ok": ok_row if ch else None})
        corr_out.append(c)
        chosen_out.append(ch or "")
    return {
        "ok": all(checks) if checks else False,
        "correct": format_dungsai_answer(corr_out),
        "chosen": format_dungsai_answer(chosen_out),
        "rows": rows,
    }


def score_dungsai(q: Dict[str, Any], user_answer: Any) -> Tuple[bool, str, str]:
    d = score_dungsai_detail(q, user_answer)
    return d["ok"], d["correct"], d["chosen"]
...
<div class="exam-login"><div style="font-size:12px;font-weight:900;color:#b45309">📢 THÔNG BÁO</div><div>Kỳ thi: <b>11 và 12/6/2026</b></div><div class="cd" id="examCdLogin">⏳ <b id="examLoginS" style="color:#dc2626;font-size:20px">--</b> giây | <span id="examLoginRest">đang tính...</span></div><div class="wish">Lớp Học Thầy Minh chúc các bạn lớp 12 thi điểm cao! 🎓</div></div>
...
const EXAM_TARGET_MS=[{label:'11/6/2026',t:Date.UTC(2026,5,11,0,0,0)},{label:'12/6/2026',t:Date.UTC(2026,5,12,0,0,0)}];
function pad2(n){return String(n).padStart(2,'0')}
function nextExamTarget(){let now=Date.now();for(const d of EXAM_TARGET_MS){if(d.t>now)return d}return null}
function tickExamLogin(){let el=document.getElementById('examCdLogin'),sEl=document.getElementById('examLoginS'),rEl=document.getElementById('examLoginRest');if(!el)return;let tgt=nextExamTarget();if(!tgt){if(sEl)sEl.textContent='00';if(rEl)rEl.textContent='Chúc các em thi tốt!';return}let ms=tgt.t-Date.now(),d=Math.floor(ms/86400000),h=Math.floor((ms%86400000)/3600000),m=Math.floor((ms%3600000)/60000),s=Math.floor((ms%60000)/1000);if(sEl)sEl.textContent=pad2(s);if(rEl)rEl.textContent='Còn '+d+' ngày '+h+' giờ '+pad2(m)+' phút → thi '+tgt.label}
...
<style>
:root{--blue:#1d4ed8;--border:#d7e0ed;--bg:#f5f7fb;--green:#dcfce7;--red:#fee2e2;--yellow:#fff7ed}*{box-sizing:border-box}body{margin:0;background:var(--bg);font-family:Arial,Helvetica,sans-serif;font-size:15px}.top{position:relative;z-index:1;background:var(--blue);color:#fff;padding:10px 14px}.headerStack{position:sticky;top:0;z-index:9;box-shadow:0 2px 8px #0002}.top h1{font-size:18px;margin:0}.top.doing h1{font-size:20px;font-weight:900;letter-spacing:.2px}.top.doing #info{font-size:15px;line-height:1.45;opacity:1}.topSub{margin-top:4px;font-size:14px;opacity:.95}.top a{color:#fff}.exam-strip{background:linear-gradient(90deg,#fef3c7,#fde68a,#fcd34d);border-bottom:3px solid #f59e0b;padding:10px 14px;display:flex;flex-wrap:wrap;gap:6px 14px;align-items:center;justify-content:center;text-align:center;line-height:1.35}.exam-strip-tag{font-size:12px;font-weight:900;color:#b45309;letter-spacing:1px}.exam-strip-dates{font-size:15px;font-weight:800;color:#78350f}.exam-strip-cd{font-size:17px;font-weight:900;color:#1d4ed8;font-variant-numeric:tabular-nums}.exam-strip-cd b{display:inline-block;min-width:1.6em;text-align:center;background:#fff;border:2px solid #93c5fd;border-radius:6px;padding:1px 6px;margin:0 2px}.exam-strip-cd b.cd-sec{min-width:2.2em;color:#dc2626;border-color:#fca5a5;font-size:19px}.exam-strip-wish{font-size:14px;font-weight:800;color:#166534}.exam-strip.done{background:linear-gradient(90deg,#dcfce7,#bbf7d0);border-color:#22c55e}.exam-strip.done .exam-strip-cd{color:#15803d}.wrap{max-width:1420px;margin:auto;padding:12px}.panel{background:#fff;border:1px solid var(--border);border-radius:12px;padding:12px;margin-bottom:12px;box-shadow:0 1px 4px #0001}.row{display:flex;gap:10px;flex-wrap:wrap;align-items:end}.field{display:flex;flex-direction:column;gap:4px;min-width:160px;flex:1}.field label{font-weight:700;font-size:12px}select,input,textarea,button{font-family:inherit;font-size:14px;border:1px solid var(--border);border-radius:8px;padding:9px 10px;background:#fff}button{cursor:pointer;font-weight:800}.btn{background:var(--blue);border-color:var(--blue);color:#fff}.btn2{background:#eef2ff;color:#1d4ed8}.btnGreen{background:#dcfce7;color:#166534}.btnRed{background:#fee2e2;color:#991b1b}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(330px,1fr));gap:12px}.card{border:1px solid var(--border);border-radius:12px;background:#fff;padding:12px}.card h3{margin:0 0 8px;color:#1e3a8a}.tag{display:inline-block;background:#eef2ff;color:#1d4ed8;padding:3px 8px;border-radius:999px;font-size:12px;font-weight:800;margin:2px}.line{height:1px;background:var(--border);margin:10px 0}.muted{color:#64748b}.hide{display:none!important}.quizLayout{display:grid;grid-template-columns:1fr 270px;gap:12px}.qid{font-size:19px;font-weight:800}.qbox{border:1px solid #111827;background:#fff;border-radius:8px;padding:14px;min-height:150px;line-height:1.55;font-size:18px}.ltx-tab{border-collapse:collapse;margin:10px auto;border:1px solid #94a3b8;font-size:16px}.ltx-tab td{border:1px solid #cbd5e1;padding:6px 14px}.opt{display:flex;gap:8px;align-items:flex-start;padding:10px;border-radius:10px;border:1px solid transparent;margin:7px 0;background:#fff}.opt:hover{background:#f8fafc}.correct{background:var(--green)!important;border-color:#86efac!important}.wrong{background:var(--red)!important;border-color:#fecaca!important}.hidden5050{opacity:.25;pointer-events:none;text-decoration:line-through}.solution{background:#fff7ed;border:1px solid #fed7aa;border-radius:10px;padding:12px;margin-top:12px}.quizTimer{display:inline-flex;align-items:center;gap:6px;background:#fef3c7;border:2px solid #f59e0b;color:#92400e;padding:8px 14px;border-radius:10px;font-size:20px;font-weight:800;font-variant-numeric:tabular-nums;min-width:110px;justify-content:center}.quizTimer.done{background:#e0e7ff;border-color:#6366f1;color:#3730a3}.navNums{display:grid;grid-template-columns:repeat(5,1fr);gap:6px}.num{padding:8px 0;border-radius:8px;border:1px solid var(--border);background:#fff}.num.active{outline:3px solid #93c5fd}.num.answered{background:#fef3c7}.num.ok{background:var(--green);color:#166534}.num.bad{background:var(--red);color:#991b1b}.tfrow{display:grid;grid-template-columns:35px 1fr 85px 85px;gap:7px;border:1px solid var(--border);border-radius:10px;padding:8px;margin:7px 0}.modal{position:fixed;inset:0;background:#0008;z-index:20;display:flex;align-items:center;justify-content:center;padding:15px}.modalBox{background:#fff;border-radius:14px;padding:16px;max-width:900px;width:100%;max-height:90vh;overflow:auto}.editGrid{display:grid;grid-template-columns:1fr 1fr;gap:10px}.editGrid textarea{min-height:80px;width:100%}.presenter{position:fixed;inset:0;z-index:99999;background:#f0f4ff;color:#111827;display:flex;flex-direction:column;padding:8px 12px 10px;overflow:hidden;width:100vw;height:100vh;height:100dvh;max-width:100vw;max-height:100dvh}.presenter.hide{display:none!important}body.presenting{overflow:hidden}body.presenting #home,body.presenting #quiz{display:none!important}body.presenting .headerStack{visibility:hidden;height:0;overflow:hidden;padding:0;margin:0;border:0;pointer-events:none}.pres-toolbar{display:flex;flex-wrap:wrap;gap:6px 10px;align-items:center;justify-content:space-between;border-bottom:2px solid #1d4ed8;padding-bottom:8px;margin-bottom:8px;flex-shrink:0}.pres-timer{display:inline-flex;align-items:center;gap:6px;background:#fef3c7;border:2px solid #f59e0b;color:#92400e;padding:8px 14px;border-radius:10px;font-size:clamp(17px,2vw,24px);font-weight:800;font-variant-numeric:tabular-nums;min-width:108px;justify-content:center;flex-shrink:0}.pres-timer.done{background:#e0e7ff;border-color:#6366f1;color:#3730a3}.pres-nav-wrap{flex-shrink:0;padding:0 0 6px;border-bottom:1px solid #cbd5e1;margin-bottom:6px}.pres-nav{display:flex;flex-wrap:wrap;gap:5px;justify-content:center;max-height:72px;overflow-y:auto;padding:2px 4px}.pres-nav .num{padding:5px 9px;min-width:34px;font-size:14px;border-radius:6px;border:1px solid #cbd5e1;background:#fff;cursor:pointer;font-weight:800;font-family:inherit}.pres-nav .num.active{outline:3px solid #1d4ed8;background:#eff6ff}.pres-nav .num.answered{background:#fef3c7}.pres-nav .num.ok{background:#dcfce7;color:#166534}.pres-nav .num.bad{background:#fee2e2;color:#991b1b}.pres-meta{font-size:clamp(15px,2vw,22px);font-weight:800;color:#1d4ed8;flex:1;min-width:160px}.pres-actions{display:flex;flex-wrap:wrap;gap:6px}.pres-frame{flex:1;min-height:0;max-height:calc(100vh - 88px);overflow-y:auto;overflow-x:hidden;border:2px solid #1d4ed8;border-radius:10px;padding:10px 14px;background:#fff}.pres-body{min-height:0}.pres-q{font-size:clamp(18px,2.2vw,28px);line-height:1.4;font-weight:600;margin-bottom:8px}.pres-img{text-align:center}.pres-img img{max-width:100%;max-height:min(28vh,240px);margin:4px auto;display:block;border:1px solid #cbd5e1;border-radius:6px}.pres-opts{display:grid;gap:8px;margin-top:6px}.pres-opt{display:flex;gap:10px;align-items:flex-start;font-size:clamp(15px,1.9vw,22px);line-height:1.35;padding:8px 10px;border:2px solid #e2e8f0;border-radius:8px;background:#f8fafc}.pres-opt.pres-row-ok{border-color:#86efac;background:#dcfce7}.pres-opt.pres-row-bad{border-color:#fecaca;background:#fee2e2}.pres-letter{flex:0 0 auto;min-width:1.4em;padding:2px 8px;text-align:center;font-weight:900;background:#1d4ed8;color:#fff;border-radius:6px;font-size:.9em}.pres-opt-text{flex:1;min-width:0;overflow-wrap:anywhere}.pres-click{cursor:pointer}.pres-click input{width:22px;height:22px;margin-top:4px;flex-shrink:0}.pres-opt.correct{border-color:#86efac!important;background:#dcfce7!important}.pres-opt.wrong{border-color:#fecaca!important;background:#fee2e2!important}.pres-tf-row{display:grid;grid-template-columns:28px 1fr auto;gap:8px;align-items:center}.pres-tf-btns{display:flex;gap:8px;flex-wrap:nowrap}.pres-tf-btn{display:flex;align-items:center;gap:6px;font-size:clamp(14px,1.6vw,20px);font-weight:800;padding:6px 12px;border:2px solid #cbd5e1;border-radius:8px;background:#fff;cursor:pointer;white-space:nowrap}.pres-tf-btn.picked-dung{border-color:#16a34a;background:#dcfce7}.pres-tf-btn.picked-sai{border-color:#dc2626;background:#fee2e2}.pres-tf-btn input{width:20px;height:20px;margin:0}.pres-short{width:100%;max-width:480px;font-size:clamp(18px,2vw,24px);padding:10px;border:2px solid #1d4ed8;border-radius:8px}.pres-short-wrap,.short-ans-wrap{display:flex;flex-wrap:wrap;gap:10px;align-items:center;margin-top:6px}.pres-short-wrap .pres-short,.short-ans-wrap input[type=text]{flex:1;min-width:200px}.pres-short-wrap .btn,.short-ans-wrap .btn{flex-shrink:0;padding:10px 18px;font-size:16px}.pres-fb{margin-top:8px;padding:10px 12px;border-radius:8px;font-size:clamp(15px,1.8vw,20px);font-weight:800;flex-shrink:0}.pres-fb.ok{background:#dcfce7;color:#166534}.pres-fb.bad{background:#fee2e2;color:#991b1b}.pres-fb.wait{background:#fef3c7;color:#92400e}.pres-hint{font-size:18px;color:#64748b;font-style:italic;padding:8px}.pres-answer,.pres-solution{font-size:clamp(16px,2vw,22px);padding:10px 12px;border-radius:8px;margin-top:8px;line-height:1.4;flex-shrink:0}.pres-answer{background:#dcfce7;border:2px solid #86efac}.pres-solution{background:#fff7ed;border:2px solid #fed7aa}.hidden5050{opacity:.22;pointer-events:none;text-decoration:line-through;filter:grayscale(1)}@media(max-width:900px){.quizLayout{grid-template-columns:1fr}.editGrid{grid-template-columns:1fr}.qbox{font-size:16px}.top h1{font-size:16px}.exam-strip-cd{font-size:15px;width:100%}}
</style></head>
<body><div class="headerStack" id="headerStack">
<div class="top" id="topBar"><h1 id="topTitle">ỨNG DỤNG LUYỆN ĐỀ VẬT LÝ - TOÁN HỌC</h1><div class="topSub"><span id="info">Đang nạp...</span> | <span id="me"></span> | <a href="/logout">Thoát</a></div></div>
<div id="examStrip" class="exam-strip">
<span class="exam-strip-tag">📢 THÔNG BÁO</span>
<span class="exam-strip-dates">Kỳ thi THPT QG 2026: <b>ngày 11 và 12/6/2026</b></span>
<span id="examCountdown" class="exam-strip-cd">⏳ Còn <b id="examD">--</b> ngày <b id="examH">--</b> giờ <b id="examM">--</b> phút <b id="examS" class="cd-sec">--</b> giây <span id="examLbl"></span></span>
<span class="exam-strip-wish">Lớp Học Thầy Minh chúc các bạn lớp 12 thi điểm cao! 🎓</span>
</div>
</div>
<div class="wrap">
...
<div id="presenter" class="presenter hide">
<div class="pres-toolbar">
<div class="pres-meta" id="presMeta">Trình chiếu</div>
<div id="presQuizTimer" class="pres-timer hide" title="Thời gian làm bài">⏱ <span id="presQuizTimerText">00:00</span></div>
<div class="pres-actions">
<button type="button" class="btn2" id="presBtnPrev" onclick="presenterGo(-1)">← Câu trước</button>
<button type="button" class="btn2" id="presBtnNext" onclick="presenterGo(1)">Câu sau →</button>
<button type="button" id="presBtn5050" class="btnGreen hide" onclick="return presUse5050(event)" title="Loại 2 đáp án sai">50-50</button>
<button id="presBtnAns" class="btnGreen hide" onclick="togglePresAnswer()">Hiện đáp án</button>
<button id="presBtnSol" class="btn2 hide" onclick="togglePresSolution()">Hiện lời giải</button>
<button class="btnRed" onclick="closePresenter()">✕ Thoát (Esc)</button>
</div></div>
<div class="pres-nav-wrap"><div class="pres-nav" id="presNav"></div></div>
<div class="pres-frame"><div class="pres-body" id="presBody"></div></div>
<div id="presFeedback" class="pres-fb hide"></div>
<div id="presAnswer" class="pres-answer hide"></div>
<div id="presSolution" class="pres-solution hide"></div>
</div>
<script>
let META=null,CATALOG=[],USER={},SID='',QUESTIONS=[],CUR=0,ANSWERS={},SUBMITTED=false,RESULTS={},CHECKED={},SOL_OPEN={},FIFTY_HIDDEN={},TIMER_IV=null,QUIZ_T0=0,QUIZ_ELAPSED=0,HOME_INFO='',CURRENT_DE=null,EXAM_CD_IV=null,PRES_ON=false,PRES_SHOW_ANS=false,PRES_SHOW_SOL=false,PRES_IGNORE_FS=false,PRES_TOAST_IV=null;
const EXAM_TARGET_MS=[{label:'11/6/2026',t:Date.UTC(2026,5,11,0,0,0)},{label:'12/6/2026',t:Date.UTC(2026,5,12,0,0,0)}];
function pad2(n){return String(n).padStart(2,'0')}
function nextExamTarget(){let now=Date.now();for(const d of EXAM_TARGET_MS){if(d.t>now)return d}return null}
function formatExamRemain(ms){if(ms<=0)return null;let d=Math.floor(ms/86400000),h=Math.floor((ms%86400000)/3600000),m=Math.floor((ms%3600000)/60000),s=Math.floor((ms%60000)/1000);return{d,h,m,s}}
function tickExamCountdown(){let tgt=nextExamTarget(),strip=document.getElementById('examStrip'),dEl=document.getElementById('examD'),hEl=document.getElementById('examH'),mEl=document.getElementById('examM'),sEl=document.getElementById('examS'),lbl=document.getElementById('examLbl');if(!sEl)return;if(!tgt){if(strip)strip.classList.add('done');if(dEl)dEl.textContent='0';if(hEl)hEl.textContent='0';if(mEl)mEl.textContent='00';if(sEl)sEl.textContent='00';if(lbl)lbl.textContent='— Chúc các em thi tốt!';return}if(strip)strip.classList.remove('done');let parts=formatExamRemain(tgt.t-Date.now());if(!parts){if(lbl)lbl.textContent='→ Sắp thi '+tgt.label+'!';return}if(dEl)dEl.textContent=String(parts.d);if(hEl)hEl.textContent=String(parts.h);if(mEl)mEl.textContent=pad2(parts.m);if(sEl)sEl.textContent=pad2(parts.s);if(lbl)lbl.textContent='→ thi '+tgt.label}
function startExamCountdown(){tickExamCountdown();if(EXAM_CD_IV)clearInterval(EXAM_CD_IV);EXAM_CD_IV=setInterval(tickExamCountdown,1000);document.addEventListener('visibilitychange',()=>{if(!document.hidden)tickExamCountdown()})}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',startExamCountdown);else startExamCountdown();
...
function isShortAns(q){return q&&(q.Dang=='Trả lời ngắn'||q.Phan=='C')}
function isQLocked(i){if(SUBMITTED)return true;if(USER.is_admin)return false;let r=getQResult(i===undefined?CUR:i);return!!r&&r.ok!=null}
function draftShortAns(){if(isQLocked(CUR))return;let q=QUESTIONS[CUR];if(!q||!isShortAns(q)||USER.is_admin)return;let el=document.getElementById(PRES_ON?'presShortAns':'shortAns');if(el)ANSWERS[CUR]=el.value}
async function confirmShortAns(){if(isQLocked(CUR))return;let q=QUESTIONS[CUR];if(!q||!isShortAns(q)||USER.is_admin)return;readAnswerFromUI();if(!answerReady(q,ANSWERS[CUR])){if(PRES_ON)showPresToast('Nhập đáp án rồi bấm Xác nhận (hoặc Enter)',true);else alert('Vui lòng nhập đáp án.');return}await checkCurrentQuestion()}
function buildShortAnsHtml(pres,dis){let id=pres?'presShortAns':'shortAns',lk=isQLocked(CUR),d=lk?'disabled':dis;return `<div class="${pres?'pres-short-wrap':'short-ans-wrap'}"><input id="${id}" class="${pres?'pres-short':''}" ${pres?'':'style="width:100%;font-size:18px"'} placeholder="Nhập đáp án..." value="${esc(ANSWERS[CUR]||'')}" ${d} ${lk?'':'oninput="draftShortAns()" onkeydown="if(event.key===\'Enter\'){event.preventDefault();confirmShortAns()}"'}><button type="button" class="btn" onclick="confirmShortAns()" ${lk?'disabled':dis}>${lk?'Đã chấm':'✓ Xác nhận'}</button></div>`}
...
async function checkCurrentQuestion(){if(USER.is_admin||isQLocked(CUR))return;let q=QUESTIONS[CUR];if(!q||q.Dang=='Tự luận')return;readAnswerFromUI();let ans=ANSWERS[CUR];if(!answerReady(q,ans)){if(CHECKED[CUR])delete CHECKED[CUR];renderNav();if(PRES_ON){updatePresFeedbackWait(q);renderPresNav()}return}try{let j=await api('/api/check-one',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({sid:SID,index:CUR,answer:ans})});if(j.graded===false)return;CHECKED[CUR]={ok:j.ok,correct:j.correct,chosen:j.chosen,details:j.details||[]};renderNav();if(PRES_ON){renderPresNav();renderPresenter()}else renderQuestion()}catch(e){}}
function readAnswerFromUI(){let q=QUESTIONS[CUR];if(!q)return;if(q.Dang=='Trắc nghiệm'&&!isDungSai(q)){let r=document.querySelector(`input[name="${PRES_ON?'pres_ans_':'ans_'}${CUR}"]:checked`);if(r)ANSWERS[CUR]=r.value}else if(isDungSai(q)){let arr=[];for(let L of ['A','B','C','D']){if(!q[L]){arr.push('');continue}let r=document.querySelector(`input[name="${PRES_ON?'pres_tf_':'tf_'}${CUR}_${L}"]:checked`);arr.push(r?r.value:'')}ANSWERS[CUR]=arr}else{let el=document.getElementById(PRES_ON?'presShortAns':'shortAns');if(el)ANSWERS[CUR]=el.value}}
function presRowDetail(r,L){if(!r||!r.details)return null;return r.details.find(d=>d.letter===L)||null}
function updatePresFeedbackWait(q){let fb=document.getElementById('presFeedback');if(!fb)return;if(isShortAns(q)&&!getQResult(CUR)){let t=ANSWERS[CUR]!=null?String(ANSWERS[CUR]).trim():'';if(t){fb.className='pres-fb wait';fb.innerHTML='Đã nhập — bấm <b>Xác nhận</b> hoặc phím Enter để chấm';fb.classList.remove('hide');return}}if(isDungSai(q)){let a=Array.isArray(ANSWERS[CUR])?ANSWERS[CUR]:[];let n=['A','B','C','D'].filter(L=>q[L]).length,m=a.filter((v,i)=>!q[['A','B','C','D'][i]]||v).length;if(m<n){fb.className='pres-fb wait';fb.innerHTML=`Đã chọn ${m}/${n} ý — chọn Đúng/Sai cho từng ý còn lại`;fb.classList.remove('hide');return}}fb.classList.add('hide')}
function renderPresFeedback(q,r){let fb=document.getElementById('presFeedback');if(!fb)return;if(!r||r.ok==null||USER.is_admin){updatePresFeedbackWait(q);return}if(isDungSai(q)&&r.details&&r.details.length){let parts=r.details.filter(d=>d.chosen).map(d=>`${d.letter}: bạn chọn <b>${esc(d.chosen)}</b>${d.ok===false?` (đúng: <b>${esc(d.correct)}</b>)`:d.ok?' ✓':''}`);fb.className='pres-fb '+(r.ok?'ok':'bad');fb.innerHTML=(r.ok?'✓ Đúng cả câu':'✗ Sai')+' — '+parts.join(' · ');fb.classList.remove('hide');return}fb.className='pres-fb '+(r.ok?'ok':'bad');fb.innerHTML=(r.ok?'✓ Đúng':'✗ Sai')+(!r.ok&&r.correct?` — Đáp án: <b>${esc(r.correct)}</b>`:'')+(r.chosen?` · Bạn chọn: <b>${esc(r.chosen)}</b>`:'');fb.classList.remove('hide')}
...
function syncQuizTimerDisplay(){let t=fmtTime(QUIZ_T0?(Date.now()-QUIZ_T0)/1000:QUIZ_ELAPSED);['quizTimerText','presQuizTimerText'].forEach(id=>{let el=document.getElementById(id);if(el)el.textContent=t})}
function syncPresTimerVisibility(){let main=document.getElementById('quizTimer'),pres=document.getElementById('presQuizTimer');if(!pres)return;let show=main&&!main.classList.contains('hide');pres.classList.toggle('hide',!show);pres.classList.toggle('done',!!(main&&main.classList.contains('done')))}
function tickQuizTimer(){if(QUIZ_T0)QUIZ_ELAPSED=(Date.now()-QUIZ_T0)/1000;syncQuizTimerDisplay()}
function startQuizTimer(){stopQuizTimer();QUIZ_T0=Date.now();QUIZ_ELAPSED=0;['quizTimer','presQuizTimer'].forEach(id=>{let box=document.getElementById(id);if(box)box.classList.remove('hide','done')});syncQuizTimerDisplay();TIMER_IV=setInterval(tickQuizTimer,1000)}
function stopQuizTimer(){if(TIMER_IV){clearInterval(TIMER_IV);TIMER_IV=null}if(QUIZ_T0){QUIZ_ELAPSED=(Date.now()-QUIZ_T0)/1000;QUIZ_T0=0}syncQuizTimerDisplay();return QUIZ_ELAPSED}
...
function buildPresImg(q){return q.HinhAnh?`<div class="pres-img"><img src="${esc(q.HinhAnh)}" referrerpolicy="no-referrer" alt=""></div>`:''}
function presInputLocked(){return isQLocked(CUR)}
function renderPresNav(){let box=document.getElementById('presNav');if(!box)return;let html='';for(let i=0;i<QUESTIONS.length;i++){let cls='num';if(i==CUR)cls+=' active';if(ANSWERS[i]!=null&&String(ANSWERS[i]).length)cls+=' answered';let r=getQResult(i);if(r&&r.ok!=null)cls+=r.ok?' ok':' bad';html+=`<button type="button" class="${cls}" onclick="presGoQ(${i})">${i+1}</button>`}box.innerHTML=html}
async function presGoQ(i){if(i<0||i>=QUESTIONS.length||i===CUR)return;await saveCurrent();CUR=i;refreshQuizView()}
function syncPresNavButtons(){let p=document.getElementById('presBtnPrev'),n=document.getElementById('presBtnNext');if(p)p.disabled=CUR<=0;if(n)n.disabled=CUR>=QUESTIONS.length-1}
function canShow5050(q){if(!q||!USER.can_5050||SUBMITTED)return false;if(isDungSai(q))return false;if(q.Dang&&q.Dang!='Trắc nghiệm'&&q.Phan!='A')return false;return['A','B','C','D'].filter(L=>q[L]).length>=2}
function is5050Used(i){return!!(FIFTY_HIDDEN[i]&&FIFTY_HIDDEN[i].length)}
function syncPres5050(q){let show=canShow5050(q),used=is5050Used(CUR);let el=document.getElementById('presBtn5050');if(el){el.classList.toggle('hide',!show);el.disabled=used||presInputLocked();el.textContent=used?'Đã dùng 50-50':'50-50'}}
function showPresToast(msg,bad){let fb=document.getElementById('presFeedback');if(!fb||!PRES_ON)return;if(PRES_TOAST_IV)clearTimeout(PRES_TOAST_IV);fb.className='pres-fb '+(bad?'bad':'ok');fb.innerHTML=esc(msg);fb.classList.remove('hide');PRES_TOAST_IV=setTimeout(()=>{if(PRES_ON&&fb)fb.classList.add('hide')},2800)}
function ensurePresenterFullscreen(){if(!PRES_ON)return;let el=document.getElementById('presenter');if(el&&!document.fullscreenElement&&el.requestFullscreen)el.requestFullscreen().catch(()=>{})}
function applyPres5050Hide(letters){(letters||[]).forEach(L=>{let pel=document.getElementById('pres_opt_'+CUR+'_'+L);if(pel)pel.classList.add('hidden5050');let el=document.getElementById('opt_'+L);if(el)el.classList.add('hidden5050')});let q=QUESTIONS[CUR];if(q)syncPres5050(q)}
function presUse5050(ev){if(ev){ev.preventDefault();ev.stopPropagation()}use5050(true);return false}
function refreshQuizView(){renderNav();if(PRES_ON){renderPresenter()}else renderQuestion()}
function buildPresOpts(q){let r=getQResult(CUR),dis=presInputLocked()||USER.is_admin?'disabled':'',showGrade=!!r&&r.ok!=null&&!USER.is_admin,html='';if(q.Dang=='Trắc nghiệm'&&!isDungSai(q)){let corr=(r&&r.correct?String(r.correct).toUpperCase().match(/[ABCD]/)?.[0]:'')||'';let chosen=r&&r.chosen?String(r.chosen).toUpperCase().match(/[ABCD]/)?.[0]||String(r.chosen):'';let hid5050=FIFTY_HIDDEN[CUR]||[];for(let L of ['A','B','C','D']){if(!q[L])continue;let chk=ANSWERS[CUR]==L?'checked':'',cls='pres-opt pres-click';if(hid5050.includes(L))cls+=' hidden5050';if(showGrade){if(corr==L)cls+=' correct';if(chosen==L&&chosen!=corr)cls+=' wrong'}html+=`<label id="pres_opt_${CUR}_${L}" class="${cls}"><input type="radio" name="pres_ans_${CUR}" value="${L}" ${chk} ${dis} onchange="saveCurrent()"><span class="pres-letter">${L}</span><div class="pres-opt-text">${renderContent(q[L])}</div></label>`}}else if(isDungSai(q)){let old=Array.isArray(ANSWERS[CUR])?ANSWERS[CUR]:['','','',''];for(let idx=0;idx<4;idx++){let L=['A','B','C','D'][idx];if(!q[L])continue;let d=presRowDetail(r,L),rowCls='pres-opt pres-tf-row';if(d&&d.ok===true)rowCls+=' pres-row-ok';else if(d&&d.ok===false)rowCls+=' pres-row-bad';let dCls=old[idx]=='Đ'?'picked-dung':'',sCls=old[idx]=='S'?'picked-sai':'';html+=`<div id="pres_opt_${CUR}_${L}" class="${rowCls}"><span class="pres-letter">${L}</span><div class="pres-opt-text">${renderContent(q[L])}</div><div class="pres-tf-btns"><label class="pres-tf-btn ${dCls}"><input type="radio" name="pres_tf_${CUR}_${L}" value="Đ" ${old[idx]=='Đ'?'checked':''} ${dis} onchange="saveCurrent()"><span>Đúng</span></label><label class="pres-tf-btn ${sCls}"><input type="radio" name="pres_tf_${CUR}_${L}" value="S" ${old[idx]=='S'?'checked':''} ${dis} onchange="saveCurrent()"><span>Sai</span></label></div></div>`}}else if(isShortAns(q)){html=buildShortAnsHtml(true,dis)}else{html='<div class="pres-hint">Câu tự luận — ghi vào vở</div>'}return html}
function renderPresenter(){let q=QUESTIONS[CUR];if(!q)return;let p=q.Phan||'D',pname=p=='A'?'Trắc nghiệm':p=='B'?'Đúng sai':p=='C'?'Trả lời ngắn':'Tự luận',r=getQResult(CUR);renderPresNav();syncPresNavButtons();document.getElementById('presMeta').textContent=`Phần ${p} (${pname}) · Câu ${CUR+1}/${QUESTIONS.length} · ${q.MucDo||''}`;document.getElementById('presBody').innerHTML=`<div class="pres-q">${renderContent(q.CauHoi)}</div>${buildPresImg(q)}<div class="pres-opts">${buildPresOpts(q)}</div>`;renderPresFeedback(q,r);syncPres5050(q);let ans=document.getElementById('presAnswer'),sol=document.getElementById('presSolution'),bA=document.getElementById('presBtnAns'),bS=document.getElementById('presBtnSol');if(USER.is_admin){bA.classList.remove('hide');bS.classList.remove('hide');bA.textContent=PRES_SHOW_ANS?'Ẩn đáp án':'Hiện đáp án';bS.textContent=PRES_SHOW_SOL?'Ẩn lời giải':'Hiện lời giải';ans.classList.toggle('hide',!PRES_SHOW_ANS);sol.classList.toggle('hide',!PRES_SHOW_SOL);if(PRES_SHOW_ANS)ans.innerHTML='<b>Đáp án:</b> '+esc(q.DapAn||'');if(PRES_SHOW_SOL)sol.innerHTML='<b>Lời giải:</b><br>'+renderContent(q.LoiGiai||'');}else{bA.classList.add('hide');bS.classList.add('hide');ans.classList.add('hide');sol.classList.add('hide')}let pr=document.getElementById('presenter');if(window.MathJax&&MathJax.typesetPromise&&pr)MathJax.typesetPromise([pr]).catch(()=>{})}
function openPresenter(){if(!QUESTIONS.length){alert('Chưa có câu hỏi.');return}PRES_ON=true;PRES_SHOW_ANS=false;PRES_SHOW_SOL=false;document.getElementById('presenter').classList.remove('hide');document.body.classList.add('presenting');syncPresTimerVisibility();syncQuizTimerDisplay();renderPresenter();let el=document.getElementById('presenter');if(el&&el.requestFullscreen)el.requestFullscreen().catch(()=>{})}
function closePresenter(){PRES_ON=false;document.getElementById('presenter').classList.add('hide');document.body.classList.remove('presenting');if(document.fullscreenElement&&document.exitFullscreen)document.exitFullscreen().catch(()=>{})}
function togglePresAnswer(){PRES_SHOW_ANS=!PRES_SHOW_ANS;renderPresenter()}
function togglePresSolution(){PRES_SHOW_SOL=!PRES_SHOW_SOL;renderPresenter()}
async function presenterGo(d){let n=CUR+d;if(n<0||n>=QUESTIONS.length)return;await saveCurrent();CUR=n;refreshQuizView()}
document.addEventListener('keydown',e=>{if(!PRES_ON)return;if(e.key==='Escape')closePresenter();else if(e.key==='ArrowRight'||e.key==='PageDown'){e.preventDefault();presenterGo(1)}else if(e.key==='ArrowLeft'||e.key==='PageUp'){e.preventDefault();presenterGo(-1)}});
document.addEventListener('fullscreenchange',()=>{if(PRES_IGNORE_FS||!PRES_ON)return});
...
async function startQuiz(made){try{let j=await api('/api/start?made='+encodeURIComponent(made));SID=j.sid;QUESTIONS=j.questions;CUR=0;ANSWERS={};CHECKED={};SOL_OPEN={};FIFTY_HIDDEN={};SUBMITTED=!!USER.is_admin;RESULTS={};document.getElementById('home').classList.add('hide');document.getElementById('quiz').classList.remove('hide');document.getElementById('resultBox').textContent=USER.is_admin?'ADMIN: đang xem đáp án/lời giải':(USER.is_trial?'DÙNG THỬ: chỉ luyện đề FREE, không chấm điểm':'');let c=CATALOG.find(x=>x.MaDe==made)||{};setTopQuizInfo(c,made);document.getElementById('quizTitle').textContent=`${c.Mon||''} ${c.Lop?'- Lớp '+c.Lop:''} | ${c.De||c.BaiHoc||''}`;startQuizTimer();renderNav();renderQuestion(); if(j.trial_message) alert(j.trial_message)}catch(e){alert('Không mở được đề: '+e.message)}}
function backHome(){closePresenter();stopQuizTimer();document.getElementById('quizTimer').classList.add('hide');let pt=document.getElementById('presQuizTimer');if(pt)pt.classList.add('hide');document.getElementById('quiz').classList.add('hide');document.getElementById('home').classList.remove('hide');CURRENT_DE=null;setTopHomeInfo(HOME_INFO)}
async function saveCurrent(){let q=QUESTIONS[CUR];if(!q||USER.is_admin)return;if(isQLocked(CUR)){renderNav();if(PRES_ON)renderPresenter();return}readAnswerFromUI();renderNav();if(isShortAns(q)&&!SUBMITTED){if(PRES_ON){updatePresFeedbackWait(q);renderPresenter()}return}await checkCurrentQuestion();if(PRES_ON){let r=getQResult(CUR);if(!r||r.ok==null)renderPresenter()}}
    ...
    try:
        ...
        if not dang or dang == "Trắc nghiệm":
            if looks_like_dungsai_answer(q.get("DapAn")) and has_tf_statements(q):
                dang = "Đúng sai"
        if dang == "Đúng sai":
            det = score_dungsai_detail(q, body.get("answer"))
            return jsonify({"ok": det["ok"], "correct": det["correct"], "chosen": det["chosen"], "details": det["rows"], "graded": True, "index": index})
        ok, correct, chosen = check_answer(q, body.get("answer"))
        c = norm_letter(correct)
        ch = norm_letter(chosen)
        details = []
        if c:
            details.append({"letter": c, "chosen": ch or "", "correct": c, "ok": bool(ch and c == ch)})
        return jsonify({"ok": ok, "correct": correct, "chosen": chosen, "details": details, "graded": True, "index": index})
    except Exception as e:

...
APP_VERSION = "V108_TN_ABCD_CIRCLE_2026_06_06"
...
function formatAbcdSolutionList(chunks,q,isDs){q=q||{};chunks=finalizeAbcdSolutionChunks(chunks,q,isDs);if(!chunks.length)return '';let corrMcq=String(q.DapAn||'').trim().toUpperCase().match(/^([ABCD])$/)?.[1];return `<div class="dsSolutionList">${chunks.map(c=>{let headVerdict='';if(c.verdict)headVerdict=`<b class="${c.verdict==='Sai'?'dsVerdictSai':'dsVerdictDung'}">${esc(c.verdict)}</b>`;else if(!isDs&&c.letter===corrMcq)headVerdict=`<b class="dsVerdictDung">✓ Đúng</b>`;else if(!isDs&&corrMcq)headVerdict=`<span class="muted">sai</span>`;let stmt=(q[c.letter]&&!c.body)?`<div class="dsStmtPreview">${renderRichText(stripOptionPrefix(q[c.letter],c.letter))}</div>`:'';let body=c.body?formatHintDisplay(c.body):'';return `<div class="dsSolutionItem"><div class="dsSolutionHead">${dsCircleHtml(c.letter)} ${headVerdict}</div>${stmt}<div class="dsSolutionBody">${body}</div></div>`}).join('')}</div>`}

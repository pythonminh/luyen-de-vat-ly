...
APP_VERSION = "V13_PRESENTER_TIMER_2026_06_01"
SCOPES = [
...
<button class="btn2" onclick="presenterGo(-1)">← Câu trước</button>
<button class="btn2" onclick="presenterGo(1)">Câu sau →</button>
...
<div class="pres-frame"><div id="pres5050Bar" class="pres-5050-bar hide"><button id="presBtn5050Frame" type="button" class="btnGreen pres-5050-btn hide" onclick="return presUse5050(event)">50-50 · Loại 2 đáp án sai</button></div><div class="pres-body" id="presBody"></div></div>
...
function presInputLocked(){let r=getQResult(CUR);return SUBMITTED||(!!r&&r.ok!=null&&!USER.is_admin)}
...
function syncPres5050(q){let show=canShow5050(q),used=is5050Used(CUR),locked=presInputLocked();['presBtn5050','presBtn5050Frame'].forEach(id=>{let el=document.getElementById(id);if(!el)return;el.classList.toggle('hide',!show);el.disabled=used||locked});let bar=document.getElementById('pres5050Bar');if(bar)bar.classList.toggle('hide',!show)}

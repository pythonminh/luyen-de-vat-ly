...
APP_VERSION = "V13_PRESENTER_5050_2026_06_01"
SCOPES = [
...
<button id="presBtn5050" class="btnGreen hide" onclick="use5050(true)" title="Loại 2 đáp án sai">50-50</button>
...
<script>
let META=null,CATALOG=[],USER={},SID='',QUESTIONS=[],CUR=0,ANSWERS={},SUBMITTED=false,RESULTS={},CHECKED={},SOL_OPEN={},FIFTY_HIDDEN={},TIMER_IV=null,QUIZ_T0=0,QUIZ_ELAPSED=0,HOME_INFO='',CURRENT_DE=null,EXAM_CD_IV=null,PRES_ON=false,PRES_SHOW_ANS=false,PRES_SHOW_SOL=false;
const EXAM_TARGET_MS=[{label:'11/6/2026',t:Date.UTC(2026,5,11,0,0,0)},{label:'12/6/2026',t:Date.UTC(2026,5,12,0,0,0)}];
function pad2(n){return String(n).padStart(2,'0')}
function nextExamTarget(){let now=Date.now();for(const d of EXAM_TARGET_MS){if(d.t>now)return d}return null}
...
async function presenterGo(d){let n=CUR+d;if(n<0||n>=QUESTIONS.length)return;await saveCurrent();CUR=n;renderQuestion();renderPresenter()}
...
document.addEventListener('fullscreenchange',()=>{if(PRES_ON&&!document.fullscreenElement){PRES_ON=false;document.getElementById('presenter').classList.add('hide');document.body.classList.remove('presenting')}});
...
async function use5050(fromPres){await saveCurrent();try{let j=await api('/api/fifty',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({sid:SID,index:CUR})});FIFTY_HIDDEN[CUR]=j.hide||[];for(let L of FIFTY_HIDDEN[CUR]){let el=document.getElementById('opt_'+L);if(el)el.classList.add('hidden5050')}document.getElementById('btn5050').disabled=true;if(j.message&&fromPres)alert(j.message);if(PRES_ON)renderPresenter();else renderQuestion()}catch(e){alert(e.message)}}

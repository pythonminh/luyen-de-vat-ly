...
<script>
let META=null,CATALOG=[],USER={},SID='',QUESTIONS=[],CUR=0,ANSWERS={},SUBMITTED=false,RESULTS={},CHECKED={},LOCKED_Q={},CURRENT_MADE='',START_IS_RETRY=false,QUIZ_ELAPSED=0,QUIZ_TIMER=null,FS_SOL_FORCE=null,FULLDE_ON=false;
...
function startQuizTimer(){stopQuizTimer();QUIZ_ELAPSED=0;let t=document.getElementById('quizTimerText');if(t)t.textContent='00:00';QUIZ_TIMER=setInterval(()=>{QUIZ_ELAPSED++;let x=document.getElementById('quizTimerText');if(x)x.textContent=fmtTime(QUIZ_ELAPSED)},1000)}
...
document.addEventListener('fullscreenchange',()=>{if(!document.fullscreenElement&&FULLDE_ON){document.body.classList.add('fullde-mode')}if(!document.fullscreenElement){FS_SOL_FORCE=null;renderQuestion()}});

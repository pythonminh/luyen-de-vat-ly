...
APP_VERSION = "V275_TRANSLATE_EN_RESTORE_2026_06_12"
...
function toggleTranslatePanel(){if(LEARNING_OPEN_KIND==='translate'){closeLearningPanel();return}LEARNING_OPEN_KIND='translate';if(!TRANSLATE_KIND)TRANSLATE_KIND='CauHoi';renderTranslatePanel();let hb=document.getElementById('hintBox');if(hb)hb.scrollIntoView({behavior:'smooth',block:'nearest'})}
function setTranslateKind(loai){loai=(loai==='LoiGiai')?'LoiGiai':'CauHoi';TRANSLATE_KIND=loai;if(LEARNING_OPEN_KIND==='translate')renderTranslatePanel()}
function renderTranslatePanel(){let hb=document.getElementById('hintBox');if(!hb||LEARNING_OPEN_KIND!=='translate')return;let qIdx=CUR;let st=translateEnStore(qIdx);let loai=TRANSLATE_KIND||'CauHoi';let data=st[loai]||{};let body='';if(TRANSLATE_LOADING){body='<div class="hintLoadingPanel"><div class="hintSpinBig"></div><div><b>Đang dịch / tải bản dịch…</b></div></div>'}else if(data.translation){let cacheNote=data.cached?'<div class="muted" style="font-size:12px;margin-bottom:6px">📋 Đã lưu Sheet Dich_Anh — gọi lại không tốn AI.</div>':'';let prov=data.provider_used?'<div class="muted" style="font-size:11px;margin-top:6px">AI: '+esc(data.provider_used)+(data.key_index?' · key #'+data.key_index:'')+'</div>':'';body=cacheNote+'<div class="translateEnPanel"><div class="hintMath">'+formatHintDisplay(data.translation)+'</div></div>'+prov}else{body='<div class="muted" style="margin-top:8px;line-height:1.5">Bấm <b>Dịch ngay</b> để xem bản tiếng Anh'+(USER.can_ai_hint?' (AI).':' — VIP/SVIP cần key Gemini để dịch mới; bản đã lưu vẫn xem được.')+'</div>'}let tabQ=loai==='CauHoi'?' translateTabOn':'';let tabL=loai==='LoiGiai'?' translateTabOn':'';let q=QUESTIONS[qIdx]||{};let canLg=canShowSolutionNow()&&!!String(q.LoiGiai||'').trim();hb.classList.remove('hide');hb.classList.add('learningOpen');hb.innerHTML='<div class="learningTitleRow"><b>🇬🇧 Dịch tiếng Anh</b><button type="button" class="learningCloseBtn" onclick="closeLearningPanel()">✕ Đóng</button></div><div class="translateEnTabs"><button type="button" class="btn2'+tabQ+'" onclick="setTranslateKind(\'CauHoi\')">📝 Đề / phương án</button>'+(canLg?'<button type="button" class="btn2'+tabL+'" onclick="setTranslateKind(\'LoiGiai\')">📗 Lời giải</button>':'')+'</div><div style="margin:8px 0"><button type="button" class="btn2 translateEnBtn" onclick="requestTranslateEn(TRANSLATE_KIND)" '+(TRANSLATE_LOADING?'disabled':'')+'>'+(TRANSLATE_LOADING?'⏳ Đang dịch…':'🔤 Dịch ngay')+'</button></div>'+body+'<div class="aiAssistWarn" style="margin-top:8px">Chỉ dịch & từ vựng — không giải bài, không chốt đáp án.</div>';syncLearningToggleUI();typesetQuizMath()}
...
def pwa_service_worker():
    js = """
const CACHE_NAME = 'luyen-de-ai-v275';

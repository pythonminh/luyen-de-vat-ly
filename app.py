...
APP_VERSION = "V121_MOBILE_GESTURE_FIX_2026_06_06"
...
def effective_dang(q: Dict[str, Any]) -> str:
    ...
    # Đáp án SSĐĐ / Đ,S,... → Đúng sai (kể cả cột J/H ghi Trắc nghiệm, kể cả thiếu cột A-D)
    if looks_like_dungsai_answer(dapan):
        return "Đúng sai"
        ...
        for item in self.catalog:
            if item.get("MaDe") == made or item.get("GroupKey") == made:
                ...
                if gk.startswith("GRP_"):
                    return list(self.by_group.get(gk, []))
                return list(self.by_made.get(gk, []))
        return []
    ...
    def hint_one(
        ...
        def _vip_detailed_hint_response(
            ...
            if not sug.get("suggested_loigiai") and sheet_loigiai:
                sug["suggested_loigiai"] = sheet_loigiai[:2000]
            note = "\n\n💎 VIP/SVIP: đáp án + lời giải đầy đủ để so sánh và ôn tập."
            hide_5050: List[str] = []
...
def build_ai_admin_review_prompt_2026(q: Dict[str, Any], user_answer: Any) -> str:
    ...
    if dang == "Đúng sai":
        dang_note = (
            "- Dạng Đúng/Sai: mục 4 BẮT BUỘC 4 tiểu mục **A.** **B.** **C.** **D.** — "
            "mỗi tiểu mục 2–5 câu giải thích vì sao Đúng/Sai (không gom chung, không bullet •); "
            "mục 5: A=Đúng/Sai, B=..., C=..., D=...; "
            "mục 8: lặp lại đủ 4 dòng **A.** … **B.** … **C.** … **D.** …"
        )
    elif dang == "Trắc nghiệm":
        dang_note = (
            "- Dạng trắc nghiệm: mục 5 kết luận một chữ A/B/C/D; "
            "mục 4 và 8 nên có 4 tiểu mục **A.** **B.** **C.** **D.** — giải thích vì sao đúng/sai từng phương án."
        )
    return "\n".join(
        [
            ...
            "- Với trắc nghiệm: giải ra kết quả rồi so A/B/C/D; tiểu mục **A.** **B.** **C.** **D.** giải thích từng phương án.",
            "- Với Đúng/Sai: BẮT BUỘC 4 tiểu mục **A.** **B.** **C.** **D.** — mỗi ý kết luận Đúng/Sai và giải thích riêng.",
            ...
            "- Với Đúng/Sai: BẮT BUỘC đủ 4 dòng **A.** Đúng/Sai — giải thích; **B.** …; **C.** …; **D.** …",
            "- Với trắc nghiệm: nên có **A.** **B.** **C.** **D.** — giải thích từng phương án, kết luận chữ đúng.",
            "- KHÔNG dùng bullet • chung chung; phải ghi rõ chữ A B C D từng ý.",
...
<body><div class="top"><div class="topRow"><h1>ỨNG DỤNG LUYỆN ĐỀ VẬT LÝ - TOÁN HỌC</h1><div class="topRight"><span id="quizTopBar" class="adminBar hide"><button type="button" class="adminTopBtn adminTopBtn2" onclick="backHome()">← Về mục lục</button></span><span id="adminBar" class="adminBar hide"><button type="button" id="syncBtn" class="adminTopBtn" onclick="syncData()">🔄 Đồng bộ Sheet</button><button type="button" id="testAiBtn" class="adminTopBtn adminTopBtn2" onclick="testServerAiKey()">🧪 Test AI</button><button type="button" id="adminEditBtn" class="adminTopBtn adminTopBtn2 hide" onclick="openEdit()">✏️ Sửa câu</button><button type="button" id="adminAddBtn" class="adminTopBtn adminTopBtn2 hide" onclick="openAddQuestion()">➕ Thêm câu</button></span><span id="info">Đang nạp...</span> | <span id="me"></span> | <button type="button" id="btnTheme" class="themeBtn" onclick="toggleTheme()" title="Chuyển giao diện tối">🌙</button> <a href="/logout">Thoát</a></div></div></div>
<div id="examStrip" class="examStrip"><span id="examMsg">📢 Thông báo kỳ thi</span><span id="examTimer" class="timer"></span></div>
<div class="wrap">
<div id="home"><div class="panel"><b>Thiết lập luyện tập</b><div class="row" style="margin-top:10px"><div class="field"><label>Môn</label><select id="fMon" onchange="onFilterChange('mon')"><option value="">Tất cả</option></select></div><div class="field"><label>Lớp</label><select id="fLop" onchange="onFilterChange('lop')"><option value="">Tất cả</option></select></div><div class="field"><label>Chương</label><select id="fChuong" onchange="onFilterChange('chuong')"><option value="">Tất cả</option></select></div><div class="field"><label>Bài học</label><select id="fBaiHoc" onchange="onFilterChange('baihoc')"><option value="">Tất cả</option></select></div><div class="field"><label>Bộ đề</label><select id="fBoDe" onchange="onFilterChange('bode')"><option value="">Tất cả</option></select></div><div class="field"><label>Mức độ</label><select id="fMucDo" onchange="onFilterChange('extra')"><option value="">Tất cả</option><option value="NB">NB</option><option value="TH">TH</option><option value="VD">VD</option><option value="VDC">VDC</option></select></div><div class="field"><label>Dạng câu</label><select id="fDang" onchange="onFilterChange('extra')"><option value="">Tất cả</option><option value="Trắc nghiệm">Trắc nghiệm</option><option value="Đúng sai">Đúng sai</option><option value="Trả lời ngắn">Trả lời ngắn</option><option value="Tự luận">Tự luận</option></select></div><div class="field"><label>Tìm nhanh</label><input id="fSearch" placeholder="Nhập từ khóa..." oninput="onFilterChange('extra')"></div><button class="btn" onclick="renderCatalog()">Lọc đề</button></div></div><div id="aiKeyPanel" class="panel hide"><b>🔑 Key AI của tôi (Gemini)</b><p class="muted" style="margin:6px 0 10px">Chỉ dùng key <b>AIzaSy...</b> từ <a href="https://aistudio.google.com/apikey" target="_blank" rel="noopener">Google AI Studio</a> (Create API key). <b>Không</b> dùng key <b>AQ...</b> hay key Google Cloud khác. Key của bạn được ưu tiên; không có thì dùng key server Render.</p><textarea id="myApiKeys" rows="3" style="width:100%;min-height:72px;font-family:Consolas,monospace" placeholder="AIzaSy...&#10;(có thể nhiều dòng — tự đổi khi hết quota)"></textarea><div class="row" style="margin-top:8px;flex-wrap:wrap"><button type="button" class="btn2" onclick="testMyAiKey()">🧪 Test key</button><button type="button" class="btnGreen" onclick="saveMyAiKey()">💾 Lưu key</button><button type="button" class="btn2" onclick="clearMyAiKey()">🗑 Xóa key của tôi</button></div><div id="aiKeyStatus" class="muted" style="margin-top:8px;font-size:13px"></div></div><div class="panel"><b>Mục lục đề</b> <span id="countCat" class="muted"></span><div id="catalog" class="grid" style="margin-top:10px"></div></div></div>
<div id="quiz" class="hide"><div class="panel row" style="justify-content:space-between"><div><span id="quizTitle" style="font-weight:800"></span> <span id="filterBadge" class="tag hide"></span> <span id="shuffleBadge" class="tag hide"></span></div><div style="display:flex;gap:10px;align-items:center"><div id="quizTimer" class="quizTimer">⏱ <span id="quizTimerText">00:00</span></div><div id="resultBox" style="font-weight:800;font-size:18px"></div></div></div><div class="quizLayout"><div><div class="panel"><div class="quizHeadRow"><div class="qid" id="qid"></div><button type="button" id="btnMobileTopToggle" class="btnMobileTopToggle" onclick="toggleMobileTopBar(event)" aria-expanded="false" title="Ẩn/hiện thanh công cụ">▾ Thanh công cụ</button><button type="button" id="btnQuizToolsToggle" class="btnQuizToolsToggle" onclick="toggleQuizTools(event)" aria-label="Công cụ" aria-expanded="false" title="Công cụ">☰</button></div><div id="quizActions" class="quizActionsPanel"><button id="btn5050" class="btnGreen" onclick="use5050()">Loại 2 câu sai</button><button id="btnHint" class="btn2" onclick="requestHint()">💡 Gợi ý AI</button><button id="btnSimilar" class="btn2 hide" onclick="requestSimilarQuestion()">📝 Tạo câu tương tự</button><button id="btnRetry" class="btnStartStrong" onclick="openRetryModal()">🔁 Làm lại đề</button><button id="btnPresent" class="btn2" onclick="toggleQuizFullscreen()">📽 Full màn hình</button><button id="btnEdit" class="btn2 hide" onclick="openEdit()">ADMIN: Sửa câu</button><button id="btnAdd" class="btnGreen hide" onclick="openAddQuestion()">➕ Thêm câu</button><button id="btnSubmit" class="btn" onclick="submitQuiz()">Nộp bài</button></div><div id="fsOnlyTools"><button class="btn2" onclick="backHome()">← Mục lục</button><div id="fsQuizTimer" class="quizTimer">⏱ <span id="fsQuizTimerText">00:00</span></div><button id="btnFsSync" class="btn2 hide" onclick="syncData()">🔄 Đồng bộ</button><button id="btnFsEdit" class="btn2 hide" onclick="openEdit()">✏️ Sửa câu</button><button id="btnFsAdd" class="btnGreen hide" onclick="openAddQuestion()">➕ Thêm câu</button><button id="btnFs5050" class="btnGreen" onclick="use5050()">Loại 2 câu sai</button><button id="btnFsHint" class="btn2" onclick="requestHint()">💡 Gợi ý AI</button><button id="btnFsSimilar" class="btn2 hide" onclick="requestSimilarQuestion()">📝 Câu tương tự</button><button class="btn2" onclick="toggleAnswerInFullscreen()">🔎 Ẩn/Hiện đáp án</button><button class="btn2" onclick="toggleExplainInFullscreen()">📘 Ẩn/Hiện lời giải</button><button type="button" id="btnFsTheme" class="btn2" onclick="toggleTheme()">🌙 Tối</button><button class="btn2" onclick="toggleQuizFullscreen()">⤢ Thoát full</button></div><div id="qtext" class="qbox"></div><div id="options"></div><div id="solution" class="solution hide"></div><div id="hintBox" class="solution hide"></div><div class="quizNavRow"><button type="button" class="btnNavMini" onclick="prevQ()" title="Câu trước" aria-label="Câu trước">‹</button><button type="button" class="btnNavWide" onclick="prevQ()">← Câu trước</button><button type="button" class="btnNavWide btnNavPrimary" onclick="nextQ()">Câu sau →</button><button type="button" class="btnNavMini btnNavPrimary" onclick="nextQ()" title="Câu sau" aria-label="Câu sau">›</button></div></div></div><div class="panel fsNavPanel"><b class="fsNavTitle">Bảng câu hỏi</b><div id="navNums" class="navNums" style="margin-top:10px"></div><div class="line"></div><div class="muted">ADMIN vào đề sẽ thấy đáp án/lời giải ngay và được sửa câu.</div></div></div></div>
</div><div id="startModal" class="modal hide"><div class="modalBox" style="max-width:520px"><h3 id="startModalTitle">Thiết lập làm bài</h3><p class="muted">Chọn cách xáo trộn để tự rèn luyện. Có thể giữ nguyên thứ tự như đề gốc.</p><p id="startFilterNote" class="hide" style="margin:8px 0;padding:8px 10px;border-radius:8px;background:#dcfce7;border:1px solid #86efac;color:#166534;font-weight:800;font-size:13px"></p><div style="display:flex;flex-direction:column;gap:10px;margin:14px 0"><label style="display:flex;gap:8px;align-items:flex-start;padding:10px;border:1px solid var(--border);border-radius:10px"><input type="checkbox" id="chkShuffleQ"> <span><b>Xáo trộn câu hỏi</b><br><span class="muted">Đổi thứ tự các câu trong đề.</span></span></label><label style="display:flex;gap:8px;align-items:flex-start;padding:10px;border:1px solid var(--border);border-radius:10px"><input type="checkbox" id="chkShuffleA"> <span><b>Xáo trộn đáp án</b><br><span class="muted">Chỉ áp dụng câu trắc nghiệm A-B-C-D.</span></span></label></div><div class="row" style="justify-content:flex-end;gap:8px"><button onclick="closeStartModal()">Hủy</button><button class="btn2" onclick="pickShufflePreset('none')">Giữ nguyên</button><button class="btn" onclick="confirmStartQuiz()">Bắt đầu</button></div></div></div><div id="modal" class="modal hide"><div class="modalBox"><h3 id="editModalTitle">ADMIN: Sửa câu hỏi</h3><div id="editForm" class="editGrid"></div><div class="row" style="justify-content:space-between;margin-top:12px"><button id="btnDeleteQuestion" class="btnRed" onclick="deleteQuestion()">Xóa câu này khỏi Google Sheet</button><div><button onclick="closeEdit()">Hủy</button><button id="btnSaveQuestion" class="btn" onclick="saveQuestionModal()">Lưu vào Google Sheet</button></div></div><div class="muted" style="margin-top:8px" id="editModalNote">Xóa liên tiếp được — app tự cập nhật số dòng Sheet, không cần đồng bộ lại sau mỗi lần xóa. Chỉ bấm Đồng bộ khi sửa trực tiếp trên Google Sheet.</div></div></div>
<script>
let META=null,CATALOG=[],USER={},SID='',QUESTIONS=[],CUR=0,ANSWERS={},SUBMITTED=false,RESULTS={},CHECKED={},LOCKED_Q={},CURRENT_MADE='',CURRENT_LEVEL='',CURRENT_DANG='',START_IS_RETRY=false,QUIZ_ELAPSED=0,QUIZ_TIMER=null,FS_ANS_FORCE=null,FS_EXP_FORCE=null,FULLDE_ON=false,FS_NAV_HIDDEN=false,COMPLETED_NOTICE=false,HINT_BY_Q={},HINT_LOADING=false,HINT_LOADING_Q=null,SIMILAR_BY_Q={},SIMILAR_LOADING=false,SIMILAR_LOADING_Q=null,MOBILE_QUIZ_TOOLS_OPEN=false,MOBILE_TOP_BAR_COLLAPSED=true,QUIZ_SCROLL_Y=0,QUESTION_MODAL_MODE='edit';
...
function resolveDang(q){if(!q)return 'Trắc nghiệm';let rawCol=dangMetaRaw(q);let dc=normDangClient(rawCol);if(dc==='Đúng sai')return 'Đúng sai';if(dc==='Trả lời ngắn')return 'Trả lời ngắn';if(dc==='Tự luận')return 'Tự luận';if(looksDsAnswer(q.DapAn))return 'Đúng sai';if(isMcqLetter(q.DapAn)&&hasOptsClient(q))return 'Trắc nghiệm';if(looksShortAnswerClient(q))return 'Trả lời ngắn';if(dc==='Trắc nghiệm')return 'Trắc nghiệm';if(rawCol)return dc;return 'Trắc nghiệm'}
...
function stripOptionPrefix(text,L){text=String(text||'').trim();if(!text||!L)return text;return text.replace(new RegExp('^\\s*'+L+'\\s*[\\.\\)\\:]\\s*','i'),'').trim()}
...
function formatDsSolutionRows(text,q){let t=String(text||'').trim();q=q||currentQuestion();if(!t&&q&&isCurrentQuestionDs())return formatAbcdSolutionList([],q,true);let chunks=extractAbcdSolutionChunks(t);if(chunks.length>=1||(q&&['A','B','C','D'].some(L=>q[L])))return formatAbcdSolutionList(chunks,q,true);return formatHintDisplay(t)}
...
function lockQuizPageScroll(){if(!isMobileQuizUI()||!isQuizVisible())return;if(document.body.classList.contains('quiz-scroll-lock'))return;QUIZ_SCROLL_Y=window.scrollY||document.documentElement.scrollTop||0;document.body.classList.add('quiz-scroll-lock');document.body.style.top=`-${QUIZ_SCROLL_Y}px`}
function unlockQuizPageScroll(){if(!document.body.classList.contains('quiz-scroll-lock'))return;document.body.classList.remove('quiz-scroll-lock');document.body.style.top='';window.scrollTo(0,QUIZ_SCROLL_Y||0)}
function getQuizMainScrollEl(){if(FULLDE_ON&&isMobileQuizUI()){let o=document.getElementById('options');if(o)return o}return document.querySelector('#quiz .quizLayout>div:first-child>.panel')||document.getElementById('quiz')}
function initMobileGestureGuard(){if(window._mobileGestureGuardInited)return;window._mobileGestureGuardInited=true;let startY=0,startX=0,tracking=false;document.addEventListener('touchstart',function(e){if(!isMobileQuizUI()||!isQuizVisible())return;if(e.touches.length!==1)return;startY=e.touches[0].clientY;startX=e.touches[0].clientX;tracking=true},{passive:true});document.addEventListener('touchmove',function(e){if(!tracking||!isMobileQuizUI()||!isQuizVisible())return;if(e.touches.length!==1)return;let y=e.touches[0].clientY,x=e.touches[0].clientX;let dy=y-startY,dx=x-startX;if(Math.abs(dx)>Math.abs(dy)+4)return;let el=getQuizMainScrollEl();let docTop=(document.scrollingElement||document.documentElement).scrollTop||0;if(el){if(el.scrollTop<=0&&dy>4){e.preventDefault();return}if(el.scrollTop+el.clientHeight>=el.scrollHeight-2&&dy<-4){e.preventDefault();return}}else if(docTop<=0&&dy>4){e.preventDefault()}},{passive:false});document.addEventListener('touchend',function(){tracking=false},{passive:true})}
function toggleMobileTopBar(e){if(e&&e.stopPropagation)e.stopPropagation();if(!isMobileQuizUI())return;MOBILE_TOP_BAR_COLLAPSED=!MOBILE_TOP_BAR_COLLAPSED;if(MOBILE_TOP_BAR_COLLAPSED)MOBILE_QUIZ_TOOLS_OPEN=false;syncMobileQuizChrome()}
function syncMobileQuizChrome(){let mobile=isMobileQuizUI();document.documentElement.classList.toggle('mobile-quiz-ui',mobile);document.body.classList.toggle('mobile-topbar-collapsed',mobile&&MOBILE_TOP_BAR_COLLAPSED);let tb=document.getElementById('btnMobileTopToggle');if(tb){tb.style.display=mobile?'inline-flex':'none';tb.textContent=MOBILE_TOP_BAR_COLLAPSED?'▾ Thanh công cụ':'▴ Ẩn thanh';tb.setAttribute('aria-expanded',MOBILE_TOP_BAR_COLLAPSED?'false':'true')}syncMobileQuizToolbar();if(mobile&&isQuizVisible())lockQuizPageScroll();else unlockQuizPageScroll()}
...
async function startQuiz(made,shuffleQ=false,shuffleA=false,level='',dang=''){try{let lv=(level||val('fMucDo')||CURRENT_LEVEL||'').trim().toUpperCase();let dg=(dang||val('fDang')||CURRENT_DANG||'').trim();let dgNorm=dg?normDangClient(dg):'';let j=await api('/api/start',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({made,shuffle_q:shuffleQ?1:0,shuffle_a:shuffleA?1:0,level:lv,dang:dgNorm||dg})});if(getShareParams().de)clearShareQuery();SID=j.sid;if(typeof j.admin==='boolean')USER.is_admin=j.admin;if(typeof j.can_view_solution_live==='boolean')USER.can_view_solution_live=j.can_view_solution_live;QUESTIONS=(j.questions||[]).map(q=>applyResolvedDang(q));if(lv||dgNorm||dg){QUESTIONS=applyQuizFilters(QUESTIONS,lv,dgNorm||dg)}if(!QUESTIONS.length){let item=CATALOG.find(x=>x.MaDe==made)||{};let mc=filterMatchCount(item,lv,dgNorm||dg);let msg='Không có câu';if(lv&&dg)msg+=` mức ${lv} dạng ${dgNorm||dg}`;else if(dg)msg+=` dạng ${dgNorm||dg}`;else if(lv)msg+=` mức ${lv}`;msg+=' trong đề này.';if(mc)msg+=`\n\nMục lục báo có ${mc} câu — bấm 🔄 Đồng bộ Sheet, Ctrl+F5, thử lại.`;else if(lv&&dg)msg+='\n\nThử bỏ Mức độ hoặc Dạng câu về Tất cả.';alert(msg);return}CURRENT_MADE=made;CURRENT_LEVEL=lv;CURRENT_DANG=dgNorm||dg||j.dang_filter||'';CUR=0;ANSWERS={};SUBMITTED=!!USER.is_admin;RESULTS={};CHECKED={};LOCKED_Q={};COMPLETED_NOTICE=false;HINT_BY_Q={};SIMILAR_BY_Q={};SIMILAR_LOADING=false;SIMILAR_LOADING_Q=null;FS_ANS_FORCE=null;FS_EXP_FORCE=null;document.getElementById('home').classList.add('hide');document.getElementById('quiz').classList.remove('hide');document.getElementById('resultBox').textContent=USER.is_admin?'ADMIN: đang xem đáp án/lời giải':(USER.is_trial?'DÙNG THỬ: chỉ luyện đề FREE, không chấm điểm':'');let c=CATALOG.find(x=>x.MaDe==made)||{};let lvTag=lv?` | Mức: ${lv}`:'';let n=QUESTIONS.length;let dgShow=CURRENT_DANG||j.dang_filter||'';let dgTag=dgShow?` | Dạng: ${dgShow} (${n} câu)`:'';document.getElementById('quizTitle').textContent=`${c.Mon||''} ${c.Lop?'- Lớp '+c.Lop:''} | ${c.De||c.BaiHoc||''}${lvTag}${dgTag}`;updateFilterBadge(lv,dgShow,n);updateShuffleBadge(j);startQuizTimer();updateAdminChrome();renderNav();renderQuestion();MOBILE_TOP_BAR_COLLAPSED=isMobileQuizUI();syncMobileQuizChrome(); if(j.trial_message) alert(j.trial_message)}catch(e){alert('Không mở được đề: '+e.message)}}
function backHome(){stopQuizTimer();FS_ANS_FORCE=null;FS_EXP_FORCE=null;MOBILE_TOP_BAR_COLLAPSED=true;MOBILE_QUIZ_TOOLS_OPEN=false;unlockQuizPageScroll();updateFilterBadge('','',null);document.getElementById('quiz').classList.add('hide');document.getElementById('home').classList.remove('hide');syncMobileQuizChrome();updateAdminChrome()}
...
function canShowSolutionNow(){return canViewSolutionLive()}
function toggleAnswerInFullscreen(){if(!FULLDE_ON)return;let can=canShowSolutionNow();if(!can){alert('Chưa đến chế độ xem đáp án.');return;}FS_ANS_FORCE=FS_ANS_FORCE===true?false:true;renderQuestion()}
function toggleExplainInFullscreen(){if(!FULLDE_ON)return;let can=canShowSolutionNow();if(!can){alert('Chưa đến chế độ xem lời giải.');return;}FS_EXP_FORCE=FS_EXP_FORCE===true?false:true;renderQuestion()}
...
async function checkCurrentQuestion(){if(SUBMITTED)return;if(USER.is_trial){alert('Tài khoản dùng thử chỉ luyện đề, không chấm đúng/sai từng câu.');return}let q=applyResolvedDang(QUESTIONS[CUR]);if(!q||q.Dang=='Tự luận')return;let ans=ANSWERS[CUR];if(ans==null)return;if(Array.isArray(ans)&&ans.every(v=>!v))return;if(String(ans).trim()==='')return;try{let j=await api('/api/check-one',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({sid:SID,index:CUR,answer:ans,...quizRestorePayload()})});CHECKED[CUR]=j;RESULTS[CUR]=j;let rb=document.getElementById('resultBox');if(j.ok){if(rb){rb.textContent=`Câu ${CUR+1}: ✅ Đúng`;rb.style.color='#166534'}}else{if(rb){rb.textContent=`Câu ${CUR+1}: ❌ Sai`;rb.style.color='#991b1b'}}renderQuestion()}catch(e){alert(e.message||'Không kiểm tra được câu này.')}}
...
function buildHintAnswerCard(j){if(!j)return '';if(!j.show_answer&&!j.vip_detailed&&!j.exact&&!j.admin_review)return '';let q=currentQuestion();let aiDa=String(j.suggested_dapan||j.correct||'').trim();let sheetDa=String(j.sheet_dapan||'').trim();let aiLg=String(j.suggested_loigiai||'').trim();let sheetLg=String(j.sheet_loigiai||'').trim();if(!sheetDa&&q)sheetDa=String(q.DapAn||'').trim();if(!sheetLg&&q)sheetLg=String(q.LoiGiai||'').trim();if(!aiDa&&sheetDa)aiDa=sheetDa;if(!aiLg&&sheetLg)aiLg=sheetLg;if(!aiDa&&!sheetDa&&!aiLg&&!sheetLg)return '';let isDs=isCurrentQuestionDs();let isTn=isCurrentQuestionTn();function fmtAns(t){if(isDs)return formatDsHintText(t,false);if(isTn)return formatTnHintText(t,false);return formatHintDisplay(t)}function fmtSol(t){if(isDs)return formatDsHintText(t,true);if(isTn)return formatTnHintText(t,true);return formatHintDisplay(t)}let rows='';if(sheetDa||aiDa){rows+=`<div class="hintAnswerRow"><b>📌 Đáp án đúng:</b> <span class="hintMath hintAnswerMain">${fmtAns(aiDa||sheetDa)}</span></div>`;if(sheetDa&&aiDa&&sheetDa.replace(/\s/g,'')!==aiDa.replace(/\s/g,''))rows+=`<div class="hintAnswerRow muted" style="font-size:13px"><b>Sheet (P):</b> <span class="hintMath">${fmtAns(sheetDa)}</span> · <b>AI:</b> <span class="hintMath">${fmtAns(aiDa)}</span></div>`}let lg=aiLg||sheetLg;if(lg)rows+=`<div class="hintAnswerRow" style="margin-top:8px"><b>📝 Lời giải:</b> <span class="hintMath">${fmtSol(lg)}</span></div>`;if(!rows)return '';return `<div class="hintAnswerCard"><div class="hintAnswerTitle">✅ Đáp án &amp; lời giải — so sánh ôn tập</div>${rows}</div>`}
function buildSheetPreviewCard(){if(!canViewSolutionLive())return '';let q=currentQuestion();if(!q)return '';return buildHintAnswerCard({show_answer:true,vip_detailed:true,sheet_dapan:q.DapAn||'',sheet_loigiai:q.LoiGiai||'',suggested_dapan:q.DapAn||'',suggested_loigiai:q.LoiGiai||''})}
function buildHintSimilarSection(){if(!USER.can_ai_hint)return '';let sim=SIMILAR_BY_Q[CUR];let loading=SIMILAR_LOADING&&SIMILAR_LOADING_Q===CUR;let html=`<div class="hintAiActions"><button type="button" class="btn2" id="btnSimilarInline" onclick="requestSimilarQuestion()" ${loading?'disabled':''}>${loading?'⏳ Đang tạo câu tương tự…':'📝 Tạo câu tương tự'}</button></div>`;if(sim&&sim.similar)html+=`<div class="hintSimilarBox" id="hintSimilarBox"><div class="hintSimilarTitle">📝 Câu tương tự (AI)</div><div class="hintSimilarBody hintMath" id="hintSimilarBody">${formatHintDisplay(sim.similar)}</div></div>`;return html}
function showHintLoadingBox(){let hb=document.getElementById('hintBox');if(!hb)return;hb.classList.remove('hide');hb.classList.add('hintBoxLoading');let title=USER.is_admin?'🔍 AI kiểm tra (ADMIN):':(USER.is_vip?'💡 AI VIP + đáp án:':'💡 Gợi ý AI:');let sub=USER.is_admin||USER.is_vip?'Gemini đang soạn đáp án + bài giải đủ 8 mục — thường 15–45 giây…':'Đang phân tích câu hỏi…';let preview=(USER.is_admin||USER.is_vip||USER.can_view_solution_live)?buildSheetPreviewCard():'';hb.innerHTML=`<b>${title}</b>${preview}<div class="hintLoadingPanel"><div class="hintSpinBig"></div><div><b>Đang gọi AI…</b><div class="muted" style="margin-top:6px;font-size:13px;line-height:1.45">${esc(sub)}</div></div></div>`;if(preview)typeset(hb.querySelector('.hintAnswerCard'))}
function renderHintBox(j){let hb=document.getElementById('hintBox');if(!hb)return;j=j||{};hb.classList.remove('hide');hb.classList.remove('hintBoxLoading');let title=j.admin_review?'🔍 AI kiểm tra (ADMIN):':(j.vip_detailed?'💡 AI VIP + đáp án:':'💡 Gợi ý AI:');if(!j.hint&&!j.admin_review&&!j.vip_detailed&&SIMILAR_BY_Q[CUR])title='📝 Câu tương tự (AI)';let extra='';if(j.admin_review){extra=`<div class="muted" style="margin-top:6px;font-size:12px">Chọn/chép nội dung bên dưới hoặc bấm nút chuyển vào Sheet</div>`;if(j.key_index)extra+=`<div class="muted" style="margin-top:4px;font-size:12px">AI: ${esc(j.provider_used||'')} | key #${j.key_index}</div>`;else if(!j.ai_configured)extra+=`<div class="muted" style="margin-top:4px;font-size:12px;color:#991b1b">Cần GEMINI_API_KEY trên Render</div>`;if(j.ai_error)extra+=`<div class="muted" style="margin-top:4px;font-size:12px;color:#991b1b">${esc(j.ai_error)}</div>`;if(j.hint_truncated)extra+=`<div class="muted" style="margin-top:4px;font-size:12px;color:#9a3412">⚠️ AI có thể chưa đủ mục 5–8 — bấm lại <b>AI kiểm tra</b> hoặc tăng GEMINI_ADMIN_MODEL trên Render.</div>`;extra+=`<div class="hintAdminActions"><button type="button" class="btn2" onclick="copyHintAll()">📋 Chép toàn bộ</button><button type="button" class="btn2" onclick="applyHintField('DapAn')">→ Đáp án (P)</button><button type="button" class="btn2" onclick="applyHintField('LoiGiai')">→ Lời giải (R)</button><button type="button" class="btnGreen" onclick="saveHintField('DapAn')">💾 Lưu đáp án Sheet</button><button type="button" class="btnGreen" onclick="saveHintField('LoiGiai')">💾 Lưu lời giải Sheet</button><button type="button" class="btn" onclick="openEditWithHint()">✏️ Mở Sửa câu</button></div>`}else if(j.vip_detailed){extra=`<div class="muted" style="margin-top:6px;font-size:12px">VIP/SVIP: đáp án + bài giải đầy đủ để so sánh và ôn tập.</div>`;if(j.key_index)extra+=`<div class="muted" style="margin-top:4px;font-size:12px">AI: ${esc(j.provider_used||'')} | key #${j.key_index}</div>`;else if(!j.ai_configured)extra+=`<div class="muted" style="margin-top:4px;font-size:12px;color:#991b1b">Chưa có key — nạp tại 🔑 Key AI của tôi</div>`;if(j.ai_error)extra+=`<div class="muted" style="margin-top:4px;font-size:12px;color:#991b1b">${esc(j.ai_error)}</div>`;if(j.hint_truncated)extra+=`<div class="muted" style="margin-top:4px;font-size:12px;color:#9a3412">⚠️ AI có thể chưa đủ mục — bấm lại <b>Gợi ý AI</b>.</div>`;if(j.hide_5050&&j.hide_5050.length)extra+=`<div class="muted" style="margin-top:4px;font-size:12px;color:#1d4ed8">Đã loại: ${esc(j.hide_5050.join(', '))}</div>`}else if(j.vip_formula_only){extra=`<div class="muted" style="margin-top:6px;font-size:12px">VIP: công thức đã thay số từ đề + tự loại 2 đáp án sai (trắc nghiệm)</div>`;if(j.hide_5050&&j.hide_5050.length)extra+=`<div class="muted" style="margin-top:4px;font-size:12px;color:#1d4ed8">Đã loại: ${esc(j.hide_5050.join(', '))}</div>`;if(j.provider_used==='FALLBACK')extra+=`<div class="muted" style="margin-top:4px;font-size:12px;color:#991b1b">AI lỗi hoặc chưa có key — <a href="#" onclick="backHome();return false">🔑 Key AI của tôi</a></div>`;else if(!j.ai_configured)extra+=`<div class="muted" style="margin-top:4px;font-size:12px;color:#991b1b">Chưa có key — nạp tại 🔑 Key AI của tôi</div>`}else if(j.key_index){extra=`<div class="muted" style="margin-top:6px;font-size:12px">AI: ${esc(j.provider_used||'')} | key #${j.key_index}</div>`}else if(j.hint){extra=`<div class="muted" style="margin-top:6px;font-size:12px">AI fallback${j.ai_configured?' (key lỗi hoặc hết quota)':' (chưa có key)'}</div>`;if(j.ai_error)extra+=`<div class="muted" style="margin-top:4px;font-size:12px;color:#991b1b">${esc(j.ai_error)}</div>`}if(j.message)extra+=`<div class="muted" style="margin-top:4px;font-size:12px">${esc(j.message)}</div>`;let answerCard=buildHintAnswerCard(j);let similarSec=buildHintSimilarSection();let body=j.hint?formatHintDisplay(j.hint):'';let bodyHtml=body?`<div id="hintAdminBody" class="hintAdminBody">${body}</div>`:'';hb.innerHTML=`<b>${title}</b>${answerCard}${extra}${bodyHtml}${similarSec}`;typeset(hb.querySelector('.hintAnswerCard'));typeset(hb.querySelector('.dsSolutionList'));typeset(document.getElementById('hintAdminBody'));typeset(document.getElementById('hintSimilarBody'))}
...
function renderQuestionForm(q){document.getElementById('editForm').innerHTML=QUESTION_FORM_FIELDS.map(f=>{let h=(f=='CauHoi'||f=='LoiGiai')?'150px':((f=='MaDe'||f=='ID'||f=='Mon'||f=='Lop'||f=='Chuong'||f=='BaiHoc'||f=='QuyenTruyCap'||f=='MucDo'||f=='Dang'||f=='DapAn'||f=='SaiSo'||f=='HinhAnh')?'56px':'78px');let val=String((q&&q[f])||'').replace(/[&<>]/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[m]));return `<div><label><b>${QUESTION_FORM_LABELS[f]||f}</b></label><textarea style="min-height:${h}" id="edit_${f}">${val}</textarea></div>`}).join('')}
function syncQuestionModalChrome(){let isAdd=QUESTION_MODAL_MODE==='add';let t=document.getElementById('editModalTitle');if(t)t.textContent=isAdd?'ADMIN: Thêm câu hỏi mới':'ADMIN: Sửa câu hỏi';let del=document.getElementById('btnDeleteQuestion');if(del)del.classList.toggle('hide',isAdd);let save=document.getElementById('btnSaveQuestion');if(save)save.textContent=isAdd?'➕ Thêm vào Google Sheet':'Lưu vào Google Sheet';let note=document.getElementById('editModalNote');if(note)note.textContent=isAdd?'Câu mới được thêm vào cuối sheet Cau_Hoi, cùng mã đề/bài học với câu hiện tại (có thể sửa trước khi lưu). Không cần đồng bộ lại sau khi thêm.':'Xóa liên tiếp được — app tự cập nhật số dòng Sheet, không cần đồng bộ lại sau mỗi lần xóa. Chỉ bấm Đồng bộ khi sửa trực tiếp trên Google Sheet.'}
function readQuestionFormData(){let data={};for(let f of QUESTION_FORM_FIELDS)data[f]=document.getElementById('edit_'+f)?document.getElementById('edit_'+f).value:'';return data}
...
def version():
    return jsonify({
        ...
        "retry_shuffle": True,
        "routes": ["/login", "/register", "/logout", "/share", "/d/<made>", "/api/meta", "/api/start", "/api/submit", "/api/question/create", "/api/question/update", "/api/question/delete"]
    })

...
APP_VERSION = "V227_ADMIN_EDIT_LEARNING_STEPS_TEST_2026_06_11"
...
function learningTitle(kind){return kind==='method'?'🧭 Phương pháp giải':'📚 Lý thuyết';}
...
function learningFieldHtml(label,val){
    ...
    if(!val)return '';
    return `<div style="margin-top:8px"><b>${esc(label)}:</b><div style="margin-top:3px;line-height:1.62;white-space:normal">${formatHintDisplay(val)}</div></div>`;
}
...
function buildEditLearningBox(q){
    ...
    return `<div style="grid-column:1/-1;margin-top:14px;padding:12px;border:2px solid #bfdbfe;border-radius:14px;background:#f8fbff">
        ...
        <div class="muted" style="font-size:12px;margin-bottom:8px">Phần này lưu vào sheet <b>Ly_Thuyet</b> và <b>Phuong_Phap</b>. Học sinh chỉ xem lại, không gọi GPT.</div>
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:10px">
            ...
            <button type="button" class="btnGreen" onclick="editGenerateLearning('theory')">🤖 GPT tạo LT + lưu</button>
        ...
        <div class="editGrid" style="grid-template-columns:repeat(2,minmax(0,1fr));gap:10px">
            <div style="padding:10px;border:1px solid #dbeafe;border-radius:12px;background:white">
                <h4 style="margin:0 0 8px 0;color:#1d4ed8">📚 Lý thuyết</h4>
            ...
            <div style="padding:10px;border:1px solid #fed7aa;border-radius:12px;background:white">
                ...
                <div class="muted" style="font-size:11px;margin-bottom:6px">PP khóa theo <b>Môn + Lớp + Chương + Bài học + Dạng bài tập cột H</b>.</div>
...
async function loadEditLearningData(){
    ...
    try{
        ...
        setEditFields('editLT_',{TieuDe:it.TieuDe,NoiDungTomTat:it.NoiDungTomTat,KienThucTrongTam:it.KienThucTrongTam,CongThuc:it.CongThuc,DonVi:it.DonVi,LuuY:it.LuuY,SaiLamThuongGap:it.SaiLamThuongGap,ViDuMau:it.ViDuMau});
        if(!isBadDangBaiTapValue(p.DangBaiTap)){
            let qsm=qs+'&DangBaiTap='+encodeURIComponent(p.DangBaiTap||'');
            let pp=await api('/api/learning/method?'+qsm,{method:'GET',timeoutMs:20000},1);
            ...
            editLearningStatus('Đã tải: LT '+((th.items||[]).length?'có':'chưa có')+' · PP '+((pp.items||[]).length?'có':'chưa có')+'.');
        }else{
...
async function saveEditLearning(kind){
    ...
    if(kind==='theory')Object.assign(item,readEditFields('editLT_',['TieuDe','NoiDungTomTat','KienThucTrongTam','CongThuc','DonVi','LuuY','SaiLamThuongGap','ViDuMau']));
...
Phương án:
{opts}
Đáp án: {clean(q.get('DapAn'))}
Lời giải: {clean(q.get('LoiGiai'))}

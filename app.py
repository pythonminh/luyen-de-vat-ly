...
APP_VERSION = "V13_PRESENTER_5050_STAY_2026_06_01"
SCOPES = [
...
<button type="button" id="presBtn5050" class="btnGreen hide" onclick="use5050(true)" title="Loại 2 đáp án sai">50-50</button>
...
<div class="pres-frame"><div id="pres5050Bar" class="pres-5050-bar hide"><button id="presBtn5050Frame" type="button" class="btnGreen pres-5050-btn hide" onclick="use5050(true)">50-50 · Loại 2 đáp án sai</button></div><div class="pres-body" id="presBody"></div></div>
...
document.addEventListener('fullscreenchange',()=>{if(PRES_IGNORE_FS||!PRES_ON)return;if(!document.fullscreenElement)setTimeout(ensurePresenterFullscreen,80)});
...
function backHome(){closePresenter();stopQuizTimer();document.getElementById('quizTimer').classList.add('hide');document.getElementById('quiz').classList.add('hide');document.getElementById('home').classList.remove('hide');CURRENT_DE=null;setTopHomeInfo(HOME_INFO)}
...
async function use5050(fromPres){if(fromPres&&PRES_ON)PRES_IGNORE_FS=true;try{if(fromPres&&PRES_ON){readAnswerFromUI();renderNav()}else await saveCurrent();let j=await api('/api/fifty',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({sid:SID,index:CUR})});FIFTY_HIDDEN[CUR]=j.hide||[];let btn=document.getElementById('btn5050');if(btn)btn.disabled=true;if(PRES_ON){renderPresenter();if(j.message)showPresToast(j.message);ensurePresenterFullscreen()}else{for(let L of FIFTY_HIDDEN[CUR]){let el=document.getElementById('opt_'+L);if(el)el.classList.add('hidden5050')}renderQuestion();if(j.message)alert(j.message)}}catch(e){if(PRES_ON&&fromPres){showPresToast(e.message,true);ensurePresenterFullscreen()}else alert(e.message)}finally{setTimeout(()=>{PRES_IGNORE_FS=false},400)}}

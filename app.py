...
APP_VERSION = "V281_TRANSLATE_TTS_PLAYER_2026_06_12"
...
.translateEnPlayer{background:linear-gradient(180deg,#f8fafc,#fff);border:1px solid #e2e8f0;border-radius:14px;padding:8px 10px;margin:8px 0;box-shadow:0 1px 3px rgba(15,23,42,.06)}
.translateEnPlayerRow{display:flex;gap:5px;align-items:center;justify-content:center;flex-wrap:nowrap}
.translateEnPlayerRow2{margin-top:6px;gap:8px}
.translateEnPlayerBtn{min-width:34px;height:34px;padding:0 7px;border-radius:999px;border:1px solid #cbd5e1;background:#fff;font-size:14px;font-weight:900;cursor:pointer;line-height:1;transition:transform .12s,box-shadow .12s,background .12s;color:#334155;flex:0 0 auto}
.translateEnPlayerBtnMain{min-width:42px;height:38px;font-size:16px;background:#dbeafe;border-color:#93c5fd;color:#1d4ed8}
.translateEnPlayerBtn.ttsBtnOn,.translateEnPlayerBtn.ttsBtnPlayOn{outline:2px solid #2563eb!important;box-shadow:0 0 0 3px #bfdbfe,0 2px 8px rgba(37,99,235,.25)!important;background:#eff6ff!important;transform:scale(1.06);color:#1d4ed8!important}
.translateEnPlayerBtn[data-tts="repeat"].ttsBtnOn{outline-color:#ca8a04!important;box-shadow:0 0 0 3px #fef08a,0 2px 8px rgba(202,138,4,.2)!important;background:#fef9c3!important;color:#854d0e!important}
.translateEnPlayerBtn[data-tts="stop"].ttsBtnOn{outline-color:#dc2626!important;box-shadow:0 0 0 3px #fecaca!important;background:#fef2f2!important;color:#b91c1c!important}
.translateEnPlayerMeta{font-size:11px;color:#64748b;font-weight:800;white-space:nowrap}
html[data-theme='dark'] .translateEnPlayer{background:linear-gradient(180deg,#0f172a,#1e293b);border-color:#334155}
html[data-theme='dark'] .translateEnPlayerBtn{background:#1e293b;border-color:#475569;color:#e2e8f0}
html[data-theme='dark'] .translateEnPlayerBtnMain{background:#1e3a5f;border-color:#3b82f6;color:#93c5fd}
@media(max-width:760px){.translateEnPlayer{padding:6px 8px;border-radius:12px}.translateEnPlayerRow{gap:4px}.translateEnPlayerBtn{min-width:30px;height:30px;font-size:12px;padding:0 5px}.translateEnPlayerBtnMain{min-width:38px;height:34px;font-size:14px}.translateEnPlayerMeta{font-size:10px}}
...
function stopTranslateEnSpeech(keepBtn){if(typeof speechSynthesis!=='undefined'){try{speechSynthesis.cancel()}catch(e){}}TRANSLATE_SPEECH_ACTIVE=false;if(!keepBtn&&!TRANSLATE_SPEECH_REPEAT)TRANSLATE_TTS_ACTIVE_BTN='';syncTranslateSpeechBtn()}
...
function buildTranslateEnPlayerBar(){let rate=(TRANSLATE_SPEECH_RATE||0.92).toFixed(2).replace(/\.00$/,'');let n=TRANSLATE_SPEECH_CHUNKS.length||0;let chunkLbl=n?((TRANSLATE_SPEECH_CHUNK_IDX+1)+'/'+n):'—';return '<div class="translateEnPlayer"><div class="translateEnPlayerRow"><button type="button" class="translateEnPlayerBtn" data-tts="prev" onclick="translateTtsPrev()" title="Câu trước">⏮</button><button type="button" class="translateEnPlayerBtn" data-tts="slower" onclick="translateTtsSlower()" title="Chậm hơn">−</button><button type="button" class="translateEnPlayerBtn translateEnPlayerBtnMain" data-tts="play" id="btnTranslateRead" onclick="translateTtsPlay()" title="Đọc / tạm dừng">'+(TRANSLATE_SPEECH_ACTIVE?'⏸':'▶')+'</button><button type="button" class="translateEnPlayerBtn" data-tts="faster" onclick="translateTtsFaster()" title="Nhanh hơn">+</button><button type="button" class="translateEnPlayerBtn" data-tts="next" onclick="translateTtsNext()" title="Câu sau">⏭</button></div><div class="translateEnPlayerRow translateEnPlayerRow2"><button type="button" class="translateEnPlayerBtn" data-tts="stop" onclick="translateTtsStop()" title="Dừng hẳn">⏹</button><button type="button" class="translateEnPlayerBtn'+(TRANSLATE_SPEECH_REPEAT?' ttsBtnOn':'')+'" data-tts="repeat" onclick="translateTtsRepeat()" title="Lặp đoạn đang đọc">🔁</button><span class="translateEnPlayerMeta" id="ttsRateLbl">x'+rate+'</span><span class="translateEnPlayerMeta" id="ttsChunkLbl">'+chunkLbl+'</span></div></div>'}
...
def pwa_service_worker():
    js = """
const CACHE_NAME = 'luyen-de-ai-v281';

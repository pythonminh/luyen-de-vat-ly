...
APP_VERSION = "V113_MOBILE_TOOLBAR_2026_06_06"
...
<body><div class="top"><div class="topRow"><h1>ỨNG DỤNG LUYỆN ĐỀ VẬT LÝ - TOÁN HỌC</h1><div class="topRight"><span id="quizTopBar" class="adminBar hide"><button type="button" class="adminTopBtn adminTopBtn2" onclick="backHome()">← Về mục lục</button></span><span id="adminBar" class="adminBar hide"><button type="button" id="syncBtn" class="adminTopBtn" onclick="syncData()">🔄 Đồng bộ Sheet</button><button type="button" id="testAiBtn" class="adminTopBtn adminTopBtn2" onclick="testServerAiKey()">🧪 Test AI</button><button type="button" id="adminEditBtn" class="adminTopBtn adminTopBtn2 hide" onclick="openEdit()">✏️ Sửa câu</button></span><span id="info">Đang nạp...</span> | <span id="me"></span> | <button type="button" id="btnTheme" class="themeBtn" onclick="toggleTheme()" title="Chuyển giao diện tối">🌙</button> <a href="/logout">Thoát</a></div></div></div>
...
function formatAbcdSolutionList(chunks,q,isDs){q=q||{};chunks=finalizeAbcdSolutionChunks(chunks,q,isDs);if(!chunks.length)return '';let corrMcq=String(q.DapAn||'').trim().toUpperCase().match(/^([ABCD])$/)?.[1];let listCls='dsSolutionList dsSolutionCompact'+(isDs?' dsSolutionDs':' dsSolutionTn');return `<div class="${listCls}">${chunks.map(c=>{let headVerdict='';if(c.verdict)headVerdict=`<b class="${c.verdict==='Sai'?'dsVerdictSai':'dsVerdictDung'}">${esc(c.verdict)}</b>`;else if(!isDs&&c.letter===corrMcq)headVerdict=`<b class="dsVerdictDung">✓ Đúng</b>`;else if(!isDs&&corrMcq)headVerdict=`<span class="muted">sai</span>`;let stmt='';if(q[c.letter]&&!c.body)stmt=`<span class="dsStmtInline">${renderRichText(stripOptionPrefix(q[c.letter],c.letter))}</span>`;let body=c.body?`<div class="dsSolutionBody">${formatHintDisplay(c.body)}</div>`:'';return `<div class="dsSolutionItem"><div class="dsSolutionHead">${dsCircleHtml(c.letter)} ${headVerdict}${stmt}</div>${body}</div>`}).join('')}</div>`}
...
function isMobileQuizUI(){return window.matchMedia('(max-width:768px)').matches}
...
function initMobileQuizToolbar(){syncMobileQuizToolbar();if(window._mobileQuizToolbarInited)return;window._mobileQuizToolbarInited=true;window.addEventListener('resize',syncMobileQuizToolbar);document.addEventListener('click',function(e){if(!isMobileQuizUI()||!MOBILE_QUIZ_TOOLS_OPEN)return;if(e.target.closest('#quizActions')||e.target.closest('#fsOnlyTools')||e.target.closest('#btnQuizToolsToggle'))return;MOBILE_QUIZ_TOOLS_OPEN=false;syncMobileQuizToolbar()})}
...
def version():
    return jsonify({
        ...
        "retry_shuffle": True,
        "routes": ["/login", "/register", "/logout", "/share", "/d/<made>", "/api/meta", "/api/start", "/api/submit", "/api/question/update", "/api/question/delete"]
    })

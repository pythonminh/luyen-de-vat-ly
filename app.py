...
APP_VERSION = "V12_PRESENTER_ANSWER_2026_06_01"
SCOPES = [
...
<script>
let META=null,CATALOG=[],USER={},SID='',QUESTIONS=[],CUR=0,ANSWERS={},SUBMITTED=false,RESULTS={},CHECKED={},SOL_OPEN={},TIMER_IV=null,QUIZ_T0=0,QUIZ_ELAPSED=0,HOME_INFO='',CURRENT_DE=null,EXAM_CD_IV=null,PRES_ON=false,PRES_SHOW_ANS=false,PRES_SHOW_SOL=false;
const EXAM_TARGET_MS=[{label:'11/6/2026',t:Date.UTC(2026,5,11,0,0,0)},{label:'12/6/2026',t:Date.UTC(2026,5,12,0,0,0)}];
function pad2(n){return String(n).padStart(2,'0')}
function nextExamTarget(){let now=Date.now();for(const d of EXAM_TARGET_MS){if(d.t>now)return d}return null}
...
function buildPresOpts(q){let dis=presInputLocked()||USER.is_admin?'disabled':'';let html='';if(q.Dang=='Trắc nghiệm'&&!isDungSai(q)){for(let L of ['A','B','C','D']){if(!q[L])continue;let chk=ANSWERS[CUR]==L?'checked':'';html+=`<label class="pres-opt pres-click"><input type="radio" name="pres_ans_${CUR}" value="${L}" ${chk} ${dis} onchange="saveCurrent()"><span class="pres-letter">${L}</span><div class="pres-opt-text">${renderContent(q[L])}</div></label>`}}else if(isDungSai(q)){let old=Array.isArray(ANSWERS[CUR])?ANSWERS[CUR]:['','','',''];for(let idx=0;idx<4;idx++){let L=['A','B','C','D'][idx];if(!q[L])continue;html+=`<div class="pres-opt pres-tf-row"><span class="pres-letter">${L}</span><div class="pres-opt-text">${renderContent(q[L])}</div><div class="pres-tf-btns"><label class="pres-tf-btn"><input type="radio" name="pres_tf_${CUR}_${L}" value="Đ" ${old[idx]=='Đ'?'checked':''} ${dis} onchange="saveCurrent()"><span>Đúng</span></label><label class="pres-tf-btn"><input type="radio" name="pres_tf_${CUR}_${L}" value="S" ${old[idx]=='S'?'checked':''} ${dis} onchange="saveCurrent()"><span>Sai</span></label></div></div>`}}else if(q.Dang=='Trả lời ngắn'){html=`<input id="presShortAns" class="pres-short" placeholder="Nhập đáp án..." value="${esc(ANSWERS[CUR]||'')}" ${dis} oninput="saveCurrent()" onblur="checkCurrentQuestion()">`}else{html='<div class="pres-hint">Câu tự luận — ghi vào vở</div>'}return html}
function renderPresenter(){let q=QUESTIONS[CUR];if(!q)return;let p=q.Phan||'D',pname=p=='A'?'Trắc nghiệm':p=='B'?'Đúng sai':p=='C'?'Trả lời ngắn':'Tự luận';document.getElementById('presMeta').textContent=`Phần ${p} (${pname}) · Câu ${CUR+1}/${QUESTIONS.length} · ${q.MucDo||''} · ${q.Dang}`;document.getElementById('presBody').innerHTML=`<div class="pres-q">${renderContent(q.CauHoi)}</div>${buildPresImg(q)}<div class="pres-opts">${buildPresOpts(q)}</div>`;let fb=document.getElementById('presFeedback'),r=getQResult(CUR);if(fb){if(r&&r.ok!=null&&!USER.is_admin){fb.className='pres-fb '+(r.ok?'ok':'bad');fb.innerHTML=(r.ok?'✓ Đúng':'✗ Sai')+( !r.ok&&r.correct?' — Đáp án đúng: '+esc(r.correct):'');fb.classList.remove('hide')}else fb.classList.add('hide')}let ans=document.getElementById('presAnswer'),sol=document.getElementById('presSolution'),bA=document.getElementById('presBtnAns'),bS=document.getElementById('presBtnSol');if(USER.is_admin){bA.classList.remove('hide');bS.classList.remove('hide');bA.textContent=PRES_SHOW_ANS?'Ẩn đáp án':'Hiện đáp án';bS.textContent=PRES_SHOW_SOL?'Ẩn lời giải':'Hiện lời giải';ans.classList.toggle('hide',!PRES_SHOW_ANS);sol.classList.toggle('hide',!PRES_SHOW_SOL);if(PRES_SHOW_ANS)ans.innerHTML='<b>Đáp án:</b> '+esc(q.DapAn||'');if(PRES_SHOW_SOL)sol.innerHTML='<b>Lời giải:</b><br>'+renderContent(q.LoiGiai||'Chưa có lời giải.')}else{bA.classList.add('hide');bS.classList.add('hide');ans.classList.add('hide');sol.classList.add('hide')}let pr=document.getElementById('presenter');if(window.MathJax&&MathJax.typesetPromise&&pr)MathJax.typesetPromise([pr]).catch(()=>{})}
...
async function saveCurrent(){let q=QUESTIONS[CUR];if(!q||USER.is_admin)return;if(q.Dang=='Trắc nghiệm'&&!isDungSai(q)){let r=document.querySelector(`input[name="ans_${CUR}"]:checked`)||document.querySelector(`input[name="pres_ans_${CUR}"]:checked`);if(r)ANSWERS[CUR]=r.value}else if(isDungSai(q)){let arr=[];for(let L of ['A','B','C','D']){let r=document.querySelector(`input[name="tf_${CUR}_${L}"]:checked`)||document.querySelector(`input[name="pres_tf_${CUR}_${L}"]:checked`);arr.push(r?r.value:'')}ANSWERS[CUR]=arr}else{let el=document.getElementById('shortAns')||document.getElementById('presShortAns');if(el)ANSWERS[CUR]=el.value}renderNav();await checkCurrentQuestion()}

...
def gemini_ping():
    ...
    if not api_key or len(api_key) < 20:
        return jsonify(ok=False, error='Chưa nhập Gemini API key hợp lệ.'), 400
    model = _model_name('gemini-2.5-flash')
    url = 'https://generativelanguage.googleapis.com/v1beta/models/' + urllib.parse.quote(model, safe='-_.') + ':generateContent?key=' + urllib.parse.quote(api_key, safe='')
    payload = {
        'contents': [{'parts': [{'text': 'Trả lời đúng một câu tiếng Việt: Key Gemini hoạt động tốt.'}]}],
    ...
    try:
        ...
        text = ''.join(p.get('text', '') for c in obj.get('candidates', []) for p in c.get('content', {}).get('parts', []) if isinstance(p, dict)).strip()
        if not text:
            return jsonify(ok=False, error='Gemini không trả lời. Key có thể sai hoặc model chưa được bật.'), 502
        return jsonify(ok=True, text='Key hoạt động. ' + text[:120])
    except urllib.error.HTTPError as e:
        raw = e.read().decode('utf-8', 'replace')
        try:
            msg = json.loads(raw).get('error', {}).get('message', raw)
